...
The combinatorial invariants of this structure---vertex count, edge topology, spectral decomposition, Euler characteristic---were not adjusted to match any prior measurement. They are necessities of the derivation. Their numerical coincidence with independently measured constants of physics, including the fine-structure constant $\alpha^{-1} \approx 137$, is the question this work places before the reader.

A derivation this compressed touches many disciplines at once—logic, combinatorics, group theory, spectral analysis, physics—and every one of them was built by someone else. The debts are large because the chain is long.
...
The compiler flags used in this work—\texttt{--safe} and \texttt{--without-K}—further restrict the environment. No external axioms can be assumed. No postulates can be introduced. No appeal to the uniqueness of identity proofs is permitted. The system is locked into the narrowest possible corridor of logical consistency, and then forced to walk.
...
\textbf{Part VI (Physics from $K_4$)} assembles the invariants into a verified computation record. The vertex count, edge structure, Euler characteristic, and spectral decomposition of $K_4$ produce numerical values that coincide with independently measured constants of physics. This is not argued. It is computed, machine-verified, and offered for the reader's evaluation.
...
The first movement closes four doors in sequence. A formal system that permits unverified postulates inherits arbitrary degrees of freedom. One without a stable notion of equality cannot refer to its own previous results. One without negation cannot state what does not hold. One without decidability cannot distinguish answered from unanswerable questions. One without a number structure cannot count its own witnesses.

None of the structures in this part are introduced as convenient machinery. Each is shown to be the unique survivor when the alternative is tested for internal consistency and fails. By the end of Part I, the logical floor is in place---and it is the only floor the constraints will support. The path from here to $K_4$ has begun.
...
Consider what happens if we do not. A formal system that permits unproven axioms—postulates accepted on faith—gains arbitrary degrees of freedom. Any structure derived from such a system inherits those degrees of freedom. The results would depend on which postulates a human happened to choose, not on what logic demands. So the first door closes: the Agda flag \texttt{--safe} forbids all postulates. Nothing enters this system that is not constructed from within.

The second door concerns identity. In many formal systems, it is taken for granted that all proofs of the same equality are themselves identical—a principle known as Axiom K. This sounds harmless. But it imposes a hidden constraint: it flattens the space of proofs into a single layer. Higher-dimensional structure—the kind where the \textit{way} two things are equal matters, not just \textit{that} they are equal—is silently collapsed before it can be examined. Since our investigation must remain open to relational complexity at every level, this premature flattening is unacceptable. The flag \texttt{--without-K} prevents it.
...
The stratification we imposed in the previous section—universe levels that prevent self-reference—creates an immediate practical problem. A proof living at one level cannot directly interact with a proof living at another. The levels are sealed compartments. Without a controlled passage between them, the system would fragment into isolated strata that cannot communicate.
...
There is a ceiling in Agda's universe hierarchy: $\mathit{Set}_\omega$, the level that sits above all numbered levels. Statements that quantify over all universe levels—and we will need such statements—live here.

But our equality type (\texttt{\_≡\_}) was defined at a specific, bounded level. It cannot operate at $\mathit{Set}_\omega$. This is not a limitation we can work around; it is a hard type error. If we leave this gap open, then the highest level of our system—the level where the most general statements live—has no notion of identity. Proofs at that level could not be compared, composed, or transported. The top would be lawless.
...
So far, every structure holds a single piece of information. But the moment we want to say something like ``there exists an element with a certain property,'' we need to hold two things at once: the element and the proof of its property. Without a way to bundle these together, existential statements are inexpressible. The system can talk about individual objects but cannot witness claims about them.
...
With pairs, we can say ``both A and B.'' But logic also requires ``either A or B''—and it requires knowing \textit{which one}. An untagged alternative—where the two cases are merged without a label—destroys the ability to distinguish them. Case analysis becomes impossible.
...
Before we can draw the first distinction—the primordial cut between ``is'' and ``is not''—we need to be precise about what it means for a type to be interesting enough to cut at all.
...
The record that follows is not a definition we have chosen. It is the minimal data that the word ``distinction'' requires in order to mean anything at all.
...
Under \texttt{--safe}, we cannot simply postulate that a distinction exists. But we can demand something weaker and logically sufficient: that the assumption of \textit{no} distinction leads to contradiction. This is double-negation stability—$\neg\neg\,\mathit{Distinction}$—the weakest assertion that still blocks the vacuous path. It does not hand us a concrete distinction. It forbids us from assuming there is none.
...
\paragraph{Where we now stand.}
The four-case classification has survived the passage from endomorphisms to maps between distinct carriers, and the resulting reachability data has been reduced to canonical presentations. The argument now controls not only what structures exist, but how their witnesses may be normalized.

\paragraph{What must happen next.}
Normalization alone does not yet isolate the invariant content. The next pressure is to separate structural observables from bookkeeping residue and force the presentation to collapse to its irreducible core.
...
Two admissible reach observables that agree on every indexed proof are observationally indistinguishable---they produce the same output on every input, so no context can separate them. The canonical normal form packs the base mediator back into admissible form, discarding whatever indexed overhead the original carried. Applying this normalization twice changes nothing: the canonical form is already base-packed, so the second pass is a no-op up to observation. The resulting iso between admissible reach observables and base reach observables is the definitive collapse: the tick index contributes no observable content to reachability, exactly as it contributed none to state.
...
With monotonicity in hand, the final step is immediate. A strict cycle $s \prec^+ s$ forces $\texttt{suc (rank}\;s\texttt{)} \le \texttt{rank}\;s$---a successor that is no larger than the number it succeeds, which \texttt{suc$\le$-impossible} refutes outright. Any drift system carrying a rank function is therefore acyclic by construction, not by fiat.
...
\textbf{Construction:} The missing global theorem is an elimination principle for $K_4$ observables: every admissible observable factors through a canonical extractor of the invariant basis already forced by the preceding chapters. In schematic form,
$$
...
\textbf{Consequence:} This would close the last gap between structural necessity and descriptive necessity. Not only would the system force the unique surviving object; it would force the only admissible way to measure it. The earlier laws already prove this collapse for indexed state observables, reach observables, and their first meta-level. What remains is the single global compression theorem stating that every admissible $K_4$ observable is exhausted by the invariant basis and nothing beyond it survives.
...
The mirror case of Law~14P.0: if a negative integer $-(1+n)$ multiplied by $x$ yields zero, then $x$ must again be zero.
...
Law 14E.0 verifies that the $\mathrm{EndoCase}$ Laplacian factors through $\mathrm{Fin4}$ indexing: the round-trip through $\mathrm{vertexAt}/\mathrm{vertexIndex}$ is the identity.
Law 14E.1 confirms that the pre-action matrix agrees with the direct Laplacian on all four indices.
...
Laws 14E.2--14E.9 confirm that all six matrix forms collapse to a single Laplacian operator.
...
Law 14E.21 establishes $L = 4I - J$ pointwise.
...
Laws 14E.23--14E.27 package this equivalence in both directions.
...
The full coupling Laplacian must equal $L_8$ pointwise. On each block, the three summands $(L_4\,v\,i + 4 \cdot v_i - \Sigma_{\text{other}})$ reassociate into $(8 \cdot v_i - \Sigma_8\,v)$ via Law~14E.10 and additive regrouping, confirming that the full survivor is exactly the complete-graph Laplacian.
...
Since $J_8$ broadcasts $\Sigma_8$ and Law~14G.12 forces that sum to zero, the image of $L_8$ lies entirely in the sum-zero subspace.
...
Each coordinate of $L_8 (L_8\,v)$ equals $8 \cdot (L_8\,v)_i - \Sigma_8 (L_8\,v)$, and Law~14G.12 forces the sum term to vanish. The minimal polynomial of $L_8$ therefore has degree at most $2$.
...
Laws 14G.16 and 14G.17 force both directions of the equivalence: no intermediate spectral case survives between sum-zero and $8$-eigen. The two predicates are definitionally packaged as one biconditional.
...
The image of $L_8$ is obtained by applying $L_8$ once, so Law~14G.15 applies immediately and pins every image vector to the $8$-eigenspace. Law~14G.16 then confirms that every sum-zero vector supplies an explicit preimage after $8$-scaling---the image and the $8$-eigenspace coincide.
...
The full survivor coincides pointwise with $L_8$ by Law~14G.8, so the sum computation reduces to Law~14G.12 directly---the full-survivor image is forced into the sum-zero subspace.
...
Because $J_8$ broadcasts the global sum of its input and Law~14G.23 forces that sum to vanish, $J_8$ annihilates every vector in the full-survivor image.
...
Under Law~14G.8 the full survivor is $L_8$, so Laws~14G.16 and 14G.17 transfer across that identification without adding new cases. Sum-zero and $8$-eigen remain the unique surviving conditions.
...
Block diagonality splits the total sum of the empty survivor into the left and right K$_4$ Laplacian sums. Law~14E.28 forces each block sum to vanish independently, so the empty-survivor image lies in the global sum-zero subspace.
...
Since $J_8$ depends only on the global sum of its input and Law~14G.27 eliminates that sum, the empty-survivor image is annihilated by $J_8$.
...
The empty survivor acts as two independent K$_4$ Laplacians. Laws~14E.11 and 14E.19 classify each block separately, leaving no mixed alternative---blockwise sum-zero and pointwise $4$-eigen are the unique surviving conditions.
...
Applying the empty survivor twice applies each K$_4$ Laplacian twice on its own block. Law~14E.29 collapses each repeated block action to $4$ times the first image, forcing every vector in the empty-survivor image into the $4$-eigenspace.
...
Law~14G.29 already forces a blockwise sum-zero vector to satisfy the empty-survivor eigenvalue equation. Choosing the vector itself as preimage eliminates any search for an additional witness---every blockwise sum-zero vector has an explicit image witness after $4$-scaling.
...
The full survivor is identified with $L_8$ both before and after one further application, so the claim reduces to Law~14G.15 by transport on both sides. Scaling congruence carries the codomain back, forcing the full-survivor image into the $8$-eigenspace.
...
Laws~14G.23, 14G.24, 14G.25, 14G.26, and 14G.34 already force all full-survivor spectral data. Product formation collects those forced components without introducing any new branch, yielding a single composite spectral witness.
...
Laws~14G.27 through 14G.32 already force the empty-survivor spectral data. The blockwise consequences and kernel witnesses are collected as one product---a single composite spectral witness for the empty survivor.
...
The survivor type has exactly two constructors by Law~14G.6. Exhaustive case split eliminates every index except \texttt{survivor-empty} and \texttt{survivor-full}, forcing a uniform survivor-indexed spectral package for every case.
...
\paragraph{Where we now stand.}
The rational measurement layer is complete enough to express the spectral content exactly. Quotients, distances, finite-index operators, coupled and triple K$_4$ Laplacians, word algebra, and rational order now describe the combinatorial data without leaving gaps at the level of explicit calculation.
...
Whether these numbers illuminate the structure of physics or merely reproduce it is a question the record itself cannot settle. But the record is exact, machine-verified, and fully open to inspection.
...
\textbf{Construction:} The present chapter proves singletonhood for the internal record \texttt{K4PhysRep}. The stronger closure theorem still to be isolated is the external completeness statement:
$$
\mathrm{RepresentationComplete} :
\bigl(\text{admissible physical representation of }K_4\bigr)
\to
...
\textbf{Consequence:} Once this closure theorem is explicit, the bridge to physics no longer says merely that one chosen representation works. It says that no rival admissible representation survives. The constants then become necessary observables of the $K_4$ symmetry chain rather than values attached by a convenient encoding.
...
The proofs are machine-checked. The correspondences are explicit. The
falsification criteria are listed above. No new prediction is claimed
here beyond what has already been extracted from the invariant record.
The sharper question is the older one: \emph{why do these numbers
match at all?}

If the correspondence is coincidental, that should be demonstrable: an
alternative structure should reproduce the same precision without
borrowing hidden freedom. If it is not coincidental, then the burden
changes. One must explain why a structure forced by elimination knows
so much about a measured world.

\section{To Mathematicians}

The framework rests on type theory, graph theory, and spectral
analysis. The formal chain is closed, but many mathematical questions
remain open:
...
observer: one who asks what remains when no appeal is allowed except
formal survival.
...
separating something from nothing. The question was never what we
would like to build on top of that mark. The question was harsher:
what remains if every unstable alternative is forced to fall away?
...
That question is not a failure of the book. It is the honest end of
the journey.
