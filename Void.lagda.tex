...
%───────────────────────────────────────────────────────────────────────────
\chapter*{Navigator}
\addcontentsline{toc}{chapter}{Navigator}
%───────────────────────────────────────────────────────────────────────────

\begin{quote}
\textit{Machine-verified in Agda (\AgdaFlag{safe} \AgdaFlag{without-K}, zero postulates, about
${\sim}25{,}000$ lines). If you are reading this to evaluate the formal claims rather than
the narrative, begin here. Every entry in the tables below is a theorem at the line
reference given; every numerical value is a \texttt{refl} closed by the Agda type checker.}
\end{quote}

Five claims thread through every chapter: (\textit{i})~the method is purely eliminative---no axiom is posited, every survivor is what destruction could not reach; (\textit{ii})~the survivor is specific---a named object, $K_4$, with determined invariants; (\textit{iii})~the invariant record is unique---proved to be a singleton, not merely shown to exist; (\textit{iv})~the arithmetic is forced---naturals, integers, rationals, and reals are theorems, not imports; (\textit{v})~the starting point is overdetermined---two independent routes converge on the same distinction $D_0$, and the final theorem loops back to it.

\subsection*{Formally Verified Claims}
...
\begin{tabular}{ll}
\toprule
\textbf{Claim} & \textbf{Section} \\
\midrule
Distinction is the unique minimal two-class type (\texttt{Two-distinction}) & \S\ref{ch:elimination-classification} \\
Exactly 4 endomorphisms on the two-class carrier (\texttt{EndoCase}, 4 constructors) & \S\ref{sec:classification:four-case-endomorphism-classification} \\
All 6 pairs are mutually distinct (6 absurdity witnesses $\lambda\,()$) & \S\ref{sec:classification:mutual-distinctness} \\
$K_4$ is the unique forced graph (\texttt{law13-1-presentation-iso}) & \S\ref{ch:graph-presentation} \\
Three simplicial weights forced to $1$; zero free parameters (\texttt{NormalizedScaleChoice})  & \S\ref{sec:scale-closure:no-free-scale} \\
$K_4$ invariant record is a singleton---no rival survives   & \S\ref{sec:k4-representation:representation-completeness} \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent\textbf{For the formalism reviewer:}
\S\ref{sec:classification:four-case-endomorphism-classification} (four-case classification)
$\to$ \S\ref{ch:graph-presentation} ($K_4$ uniqueness)
$\to$ \S\ref{sec:k4-representation:representation-completeness} (singleton record).

\medskip
\noindent\textbf{For the philosopher:}
\S\ref{sec:scale-closure:measurement-kernel} (measurement presupposes distinction)
$\to$ \S\ref{sec:discrete-continuum:record-presupposes-distinction} (the loop closes).

\medskip
\noindent\textbf{Scope of the claim.} The graph evaluation $\mathcal{C} = 137$, the tower product $P = 1836$, and the eigenvalue ratio $3{:}1$ are exact \texttt{refl} equalities derivable from the $K_4$ invariants $(V,E,d,\chi,\kappa) = (4,6,3,2,8)$ with no free parameters. No value was adjusted; each is the definitional output of the graph's combinatorial data. Whether these invariants carry further significance beyond the formal chain is a separate question, addressed in the companion volume \textit{Form}.
...
The void is the unmarked state. It precedes axioms, precedes distinction, precedes even the question of whether anything exists. But the void is not stable—because any system capable of examining itself already distinguishes examination from non-examination. The unmarked state, the instant it is interrogated, marks itself.
...
The claim here is narrower and harder: \emph{the construction that produces 137 contains no adjustable parameter, and every step is machine-verified.} The machine-checked proof that $V^d \cdot \chi + d^2$ is the unique admissible survivor in the bounded evaluation sector, together with the epistemic census of all derived sectors, appears in the companion volume \textit{Form}.
...
in the companion volume \textit{Form}.
...
combination---the proof that no other degree-$\le 3$ expression in the invariant
basis reaches $137$---is established in the companion volume \textit{Form}.
Agda requires the computation first and the classification later, because the
classification depends on the full invariant ledger assembled across the intervening
chapters.
