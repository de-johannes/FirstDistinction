...
\begin{abstract}
\noindent
Two independent formal derivations, mechanically verified in Agda under
\texttt{-{}-safe -{}-without-K} with zero postulates, produce the complete
graph $K_4$ as the unique surviving structure from a single binary distinction.
\textit{Void}~\cite{wielsch2026void} reaches $K_4$ by elimination
(${\sim}27{,}000$ lines); \textit{Form}~\cite{wielsch2026form} reaches it
by construction (${\sim}25{,}000$ lines).  The two developments share no code.
...
The formal chain from distinction to $K_4$ is presented in the companion
...
\begin{enumerate}[nosep]
  ...
  \item Arithmetic, spectral theory, and physics constants are then
    derived from $K_4$'s combinatorial invariants.
\end{enumerate}

Steps 1--4 are verified independently in both \textit{Void} and
\textit{Form}.  Step~5 extends these into the numerical record
presented below.
...
Postulates             & 0 (zero), in both volumes \\
Library imports        & None beyond \texttt{Agda.Primitive} \\
Volume 1               & \texttt{FirstDistinction.lagda.tex} (\textit{Void}), $\sim$27\,000 lines \\
Volume 2               & \texttt{FirstConstruction.lagda.tex} (\textit{Form}), $\sim$25\,000 lines \\
Shared code            & None.  Modules do not import from each other. \\
...
\begin{enumerate}[nosep]
  \item Clone: \texttt{git clone https://github.com/de-johannes/FirstDistinction}
  \item Install Agda $\ge$ 2.6.4
  \item Compile: \texttt{agda -{}-safe -{}-without-K FirstDistinction.lagda.tex}
  \item Compile: \texttt{agda -{}-safe -{}-without-K FirstConstruction.lagda.tex}
...
The combinatorial invariants of $K_4$---forced from a single binary distinction
by two independent machine-verified proof chains---evaluate to a closed set
of values that agree with measured constants of physics at sub-ppm precision.
