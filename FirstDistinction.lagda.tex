...
{\Large\itshape A Logical Ontology Of Distinction}\\[2cm]
...
Logic admits a single irreducible distinction: true or false. From this binary structure, a strict constraint follows: any system that produces a contradiction under its own requirements cannot persist.

This work applies that constraint formally and mechanically. Each step is verified in Agda—no external axioms, no postulates, no assumptions beyond what logical consistency itself demands. Every structure is justified solely by the elimination of alternatives that fail.

The result is not constructive in the traditional sense. It is reductive: a progressive filtration of everything that does not survive its own internal requirements.

What remains—$K_4$—is not introduced as a model or hypothesis. It is the minimal structure that elimination cannot touch.

The combinatorial invariants of this structure---vertex count, edge topology, spectral decomposition, Euler characteristic---were not adjusted to match any prior measurement. They are necessities of the derivation. Their numerical values can then be compared with independently measured constants of physics, including the fine-structure constant $\alpha^{-1} \approx 137$. That comparison is not part of the derivation; it is the interpretive question this work places before the reader.
...
Physics asks: which laws survive observation?
Logic asks: which statements can be decided at all?

These appear to be fundamentally different questions. They use different languages. They live in different departments. Their practitioners rarely read each other's journals. And yet, when pushed far enough, all three arrive at the same boundary: each discipline possesses an internal filter that it did not choose. Mathematics eliminates structures that contradict themselves. Physics eliminates theories that violate observed invariants. Logic eliminates propositions that cannot be assigned a truth value without inconsistency.

The mechanism is the same everywhere. What cannot stand under its own requirements, falls. The stages are different. The verdict is identical.

This raises an uncomfortable question: if the filter is the same, could the \textit{survivor} also be the same?

\section{Elimination as Method}
\label{sec:introduction:elimination-as-method}

The standard approach in formal work is constructive. One posits axioms, defines structures, proves theorems, and builds upward. The direction of reasoning is from assumption to consequence.

This work reverses that direction entirely.

Nothing is assumed. Nothing is postulated. Instead, a single starting point is taken—the most primitive act of logic: drawing a distinction, separating what \textit{is} from what \textit{is not}—and then a relentless filter is applied. At every stage, we ask: which structures survive their own consistency requirements? Those that do not are eliminated. Those that do, persist—not by our choice, but because elimination could not reach them.

This is not a new idea. It is how logic has always worked. But the full consequences of applying this filter \textit{mechanically}, from the ground up, without exception, have not been traced before.

\section{The Tool: Agda as Executioner}
\label{sec:introduction:agda-as-executioner}

A filter is only as trustworthy as its enforcement. Human reasoning is fallible: we overlook edge cases, confuse sufficiency with necessity, and mistake intuition for proof. This is why every step in this work is formalized and verified in Agda, a dependently typed proof assistant.
...
When the proofs become detailed, the global line of force is easy to lose from view. The argument itself has a single irreversible spine:
...
Before entering the proofs, the reader deserves a compact ledger of the entire eliminative chain. At every step below, the column on the right names the alternatives that were tested and destroyed. $K_4$ is what remains after every row has been executed. Nothing in this list was chosen; everything was forced by the failure of its rivals.
...
\midrule
1 & Distinction ($D_0$)
  & The bare act of separating $\ell$ from $r$. Any formal system that cannot distinguish true from false is vacuous.
...
8 & Physics record
  & The combinatorial invariants of $K_4$ ($V=4$, $E=6$, $\chi=7$, $\mathrm{Spec}=\{0,4,4,4\}$) yield $\alpha^{-1}=137$ by evaluation. No free parameter.
  & Any alternative constant that would require a different graph or a tunable parameter. \\
\bottomrule
\end{tabular}
\end{center}

The protocol is the backbone of the argument. If any single row could be replaced by a genuine alternative---if, at any step, the eliminated rival could survive instead---then the derivation would branch and the uniqueness claim would fail. The remainder of the book is the proof that no such branch exists.

This book proceeds in six parts. The first four derive $K_4$ through elimination. The fifth builds the arithmetic its structure makes necessary. The sixth computes the physical constants its invariants determine.

\textbf{Part I (Foundation)} establishes the minimal infrastructure that any consistent formal system must carry: equality, pairing, negation, decidability, and the natural numbers. None is introduced by fiat. Each is shown to be the unique survivor when the alternative is tested for internal consistency and fails.

\textbf{Part II (Drift and Reachability)} discovers what happens when classification is applied to classification itself. Each level of the type hierarchy is forced by the unclassified residue of the level below it. The reach proofs in this part track that ascent through the tower.

\textbf{Part III (Heteromorphisms and Presentation)} extends the classification to maps between two distinct carriers---not just within one. The same four cases survive. The presentation theorems then reduce every reachability fact to a finite canonical witness.

\textbf{Part IV (Observables and Fixpoint)} identifies which quantities that functionals can measure on proofs are actually determined by structure rather than proof-choice. The answer is a fixpoint: every admissible observable factors through a unique base computation. The K$_4$ graph---four vertices, six edges, complete---is the result of applying this filter to everything derived so far.

\textbf{Part V (Derived Arithmetic)} builds the number systems that K$_4$'s structure demands. Integers arise from orientation. Rationals arise from spectral ratios. Reals close the Cauchy gaps. Measurement, scaling, and distance laws follow---all without any postulate beyond what Parts I through IV established.

\textbf{Part VI (Physics from $K_4$)} assembles the invariants into a verified computation record. The vertex count, edge structure, Euler characteristic, and spectral decomposition of $K_4$ produce numerical values that can then be compared with independently measured constants of physics. The computation is theorem; the comparison is interpretation. The former is proved, machine-verified, and offered first. The latter is left open to the reader's evaluation.

The chapters do not wander away from this chain. They only slow down at different points in order to verify that no weaker alternative survives. The reader is invited to verify every claim. The source of this book is itself the proof: a literate Agda file where prose and machine-checked code are one and the same document.

\section{How To Read This Book}
\label{sec:introduction:how-to-read}

This document can be entered from more than one side.

\textbf{For the reader following the main argument:} read the Abstract, this Introduction, the opening paragraphs of each Part, \emph{Graph Presentation}, \emph{An Unexpected Correspondence}, \emph{K\textsubscript{4} Physics Constants Layer}, and the Conclusion. That path carries the global claim from distinction to $K_4$ to the physics record without demanding that every intermediate proof be read in full on the first pass.

\textbf{For the reader testing the formal necessity:} follow the code blocks and their immediate narrative wrappers. The prose states what pressure is being applied; the Agda code shows exactly which constructions survive it.

\textbf{For the reader testing the physics claim first:} begin with \emph{Graph Presentation}, then \emph{An Unexpected Correspondence}, then the openings of \emph{Derived Arithmetic} and \emph{Physics from $K_4$}, and only then descend into the intermediate derivations as needed. The point is not to delay the claim, but to make clear which earlier chapters each later computation depends on.

What begins now is the first irreversible pressure test. The question is no longer whether distinction can be described, but whether it can survive without borrowing hidden structure from outside itself. If that survival fails here, everything later collapses into rhetoric. If it holds, the path to $K_4$ is no longer a vision. It is a sentence already being executed.
...
Before a single distinction can be drawn, the environment in which it is drawn must itself be constrained. This is the first and perhaps most counterintuitive step: we do not begin by building. We begin by closing doors.

Consider what happens if we do not. A formal system that permits unproven axioms—postulates accepted on faith—gains arbitrary degrees of freedom. Any structure derived from such a system inherits those degrees of freedom. The results would depend on which postulates a human happened to choose, not on what logic demands. So the first door closes: the Agda flag \AgdaFlag{safe} forbids all postulates. Nothing enters this system that is not constructed from within.

The second door concerns identity. In many formal systems, it is taken for granted that all proofs of the same equality are themselves identical—a principle known as Axiom K. This sounds harmless. But it imposes a hidden constraint: it flattens the space of proofs into a single layer. Higher-dimensional structure—where the \textit{way} two things are equal matters, not just \textit{that} they are—is silently collapsed before it can be examined. Since our investigation must remain open to relational complexity at every level, this premature flattening is unacceptable. The flag \AgdaFlag{without-K} prevents it.
...
These three constraints—no postulates, no identity collapse, no self-reference—are not design choices. They are the minimal conditions under which a formal system can reason about its own distinctions without contradiction. The module therefore opens under the same two restrictions just named. The code block that follows is the direct translation of these constraints into Agda.
...
With the environment locked down, the first question is immediate: when are two things the same?
...
The stratification we imposed in the previous section—universe levels that prevent self-reference—creates an immediate practical problem.
...
If the filter we described in the introduction is to have any teeth, it must be possible to express that something \textit{cannot} exist. A system that can only affirm—never deny—has no elimination power. It accepts everything. And a system that accepts everything proves nothing.
...
The record \texttt{Distinction} now exists as a type. But types can be empty. If no inhabitant of this type can be constructed—if no actual distinction exists—then every law we derive from it holds vacuously. ``For all distinctions $d$, $P(d)$'' is trivially true when there are no distinctions. The entire formal apparatus would prove everything and say nothing.

This is the empty-ontology loophole, and it must be closed before any subsequent reasoning has force.

Under \AgdaFlag{safe}, we cannot simply postulate that a distinction exists. But we can demand something weaker and logically sufficient: that the assumption of \textit{no} distinction leads to contradiction. This is double-negation stability—$\neg\neg\,\mathit{Distinction}$—the weakest assertion that still blocks the vacuous path. It does not hand us a concrete distinction. It forbids us from assuming there is none.
...
The non-vacuity law says exactly the same thing in its sharpest form: distinction cannot be eliminated. The negation of distinction is itself refutable. This is not a constructive existence claim—we do not hold a distinction in our hands—but it is enough to guarantee that every subsequent argument has a non-trivial domain.
...
The distinction record is now stable under reconstruction: it can be decomposed into its four components and reassembled without loss. This is the sharpest form of structural identity the type system can express. The object is not merely described---it is locked.

\chapter{Elimination and Classification}
\label{ch:elimination-classification}

The distinction record is now established. What can be done with it?
...
Coverage has a powerful consequence: it gives us a canonical eliminator. For any predicate $P$ on the carrier, if we know $P(\ell)$ and $P(r)$, then we know $P(x)$ for every $x$. The proof is mechanical: ask \texttt{cover} which class $x$ belongs to, then substitute the appropriate witness. This is not a convenience—it is the only way to discharge a universal statement over the carrier. The two boundary cases are all there is.
...
When are two endomorphisms the same? The answer is forced by the elimination structure we already have. Since every carrier element is determined by its boundary class, and every endomorphism is determined by what it does to the boundary, two endomorphisms are equal precisely when they agree on every input. This pointwise equality is the only comparison that respects the boundary structure. Intensional distinctions—cases where two maps compute differently but produce identical results—are irrelevant. What matters is the output at every point, nothing else.
...
\paragraph{Where we now stand.}
The primitive distinction has been stabilized into explicit logical form. Equality, negation, decidability, witness structure, and the four endomorphism cases now exist as machine-checked constraints on a two-element carrier. The argument has reached its first rigid plateau: the base classification is no longer in question.

\paragraph{What must happen next.}
...
The classification is complete at a single level: every endomorphism on a distinction carrier falls into one of four cases. But the classification itself—the type \texttt{EndoCase}, the classifier function, the interpretation—is a new piece of mathematical structure. And this new structure is not yet classified. If we stop here, we leave an unclassified object in our ontology, which is exactly the kind of residual freedom our filter is designed to eliminate.

The resolution is forced: classify the classifier. But to do so, the carrier must grow—the new classification lives at a higher universe level, because it quantifies over the objects of the previous level. This is drift: the canonical step from one level to the next, carrying the old distinction into a lifted carrier that can accommodate the expanded ontology.

The drift step is not a construction we invent. It is the minimal response to a structural demand: every classification must itself be classifiable, and the type-theoretic hierarchy of universe levels is the unique mechanism that makes this possible without circularity.
...
Drift is now established: it lifts any distinction one universe level higher, preserving the boundary and coverage structure verbatim. The question that remains is whether this lift is safe---whether it distorts the carrier it inherits.
...
Drift is universal: every boundary-preserving lift factors through it, and the factorization is unique. No alternative lift can bypass the drift step. The tower is therefore canonical---what remains is to trace how far the chain reaches.
...
Reachability is now explicit: every finite chain extends, strict successors are comparable, and the eliminator is forced. The chain stretches forward without bound. The last structural hazard is circularity---whether the chain could loop back and collapse the hierarchy it has built.

\chapter{Drift Acyclicity}
\label{ch:drift-acyclicity}

A tower that never stops climbing is not safe merely because it grows. If a chain of drift steps could fold back on itself---returning to a state already visited---then the hierarchy would collapse into a finite loop, and the infinite ascent would be an illusion. Everything established so far would apply only to a cycle, not to a genuine progression. The elimination strategy demands that the tower be rigid: once a state is left behind, it is left behind permanently.
...
\paragraph{Where we now stand.}
The climb through the universe hierarchy is now explicit. Drift does not terminate, reachability records the ascent, and acyclicity prevents the hierarchy from folding back onto itself. The argument has gained vertical structure without losing rigidity.

\paragraph{What must happen next.}
That is still not enough. If the four surviving cases depended on the source and target being the same carrier, then the classification would be parochial rather than structural. The next task is to remove that loophole.

\part{Heteromorphisms and Presentation}

The classification established in Part~I was confined to maps from a distinction carrier to itself. The question now is whether relaxing that restriction changes anything. It does not. Maps between two distinct carriers face the same combinatorial constraints that produced four endomorphisms, and exactly four cases survive here as well.

This consistency matters. It confirms that the classification is not an artefact of the self-mapping assumption but a structural property of a two-element distinction, regardless of which carrier provides the domain and which the codomain. The presentation theorems that follow press this further: every reachability fact between levels reduces to a finite canonical record, and every canonical record to its minimal representative.

\chapter{Heteromorphisms}
\label{ch:heteromorphisms}

So far, classification has been confined to endomorphisms—maps from a single distinction carrier to itself. But nothing in the argument is specific to the source and target being the same. A map $f : S_{d_1} \to S_{d_2}$ between two distinct distinction carriers is just as rigidly constrained: its behavior is determined by its values at the two boundary points of the source, and each of those values is forced into one of the two boundary classes of the target. The same $2 \times 2$ combinatorics applies. The same four cases survive.
...
The four-case classification has now been extended from endomorphisms to arbitrary maps between distinct carriers. The same four cases survive, and the same uniqueness holds. The classification is structural, not an artefact of the self-mapping assumption.
...
\paragraph{Where we now stand.}
The four-case classification has survived the passage from endomorphisms to maps between distinct carriers, and the resulting reachability data has been reduced to canonical presentations. The argument now controls not only what structures exist, but how their witnesses may be normalized. The number four has reappeared at every stage---not because the argument was steered toward it, but because no other cardinality survives the constraints. The graph that four mutually distinct elements demand is the complete graph $K_4$, and its emergence is now imminent.
...
Two proofs of the same proposition may differ entirely in their internal structure while agreeing on every observable consequence. Where this happens systematically, the observable is not measuring the proof---it is measuring what the proof is about.

This part identifies which observables on reachability proofs have that property: the admissible ones, determined solely by the underlying path rather than the bookkeeping that traced it. The answer converges to a fixpoint. Every admissible observable factors through a base computation that discards staging, tick-indexing, and witness detail, retaining only the irreducible logical content. The K$_4$ graph presentation at the close of this part is that content---the single minimal certificate of everything the four preceding parts established.
...
So far, acyclicity has been asserted as a constraint: no state reaches itself through a non-empty path. Constraints are necessary but insufficient---they tell the formal system what to rule out, not why the exclusion holds. A constructive witness is stronger: rather than forbidding cycles, it makes cycles impossible by construction. The ranking principle provides that witness.

Acyclicity has so far been stated as an axiom inside the \texttt{DriftAcyclic} record: no state can reach itself via a non-empty path. But axioms invite suspicion---can the constraint actually be satisfied? A rank function settles the question. If every drift step strictly increases a natural-number rank, then any non-empty cycle $s \prec^+ s$ would force $\texttt{suc (rank}\;s\texttt{)} \le \texttt{rank}\;s$, which is impossible in $\mathbb{N}$. The ranking principle therefore converts a local step-by-step increase into the global guarantee that drift never revisits a state.
...
\textbf{Construction:} The global theorem is an elimination principle for $K_4$ observables: every admissible observable factors through a canonical extractor of the invariant basis already forced by the preceding chapters. In abstract form this is exactly the factorization law proved below for meta-observables. In the concrete $K_4$ specialization carried later in the manuscript, the global admissible sector collapses further to the four canonical observable classes boundary, correlator, observer, and channels. In schematic form,
$$
...
This is the chapter the entire argument has been converging toward.

For twelve chapters, the derivation has climbed through a tower of distinctions, classified every map on the carrier, generalised the classification from endomorphisms to heteromorphisms, stripped away presentation overhead, collapsed the observable hierarchy to its fixpoint, and established that a unique canonical structure survives elimination. That structure is now named: it is the complete graph on four vertices, $K_4$.

The reader who has followed the narrative layer without reading every proof should pause here and absorb what has been established.
...
The numerical weight of this graph will be explored in the next chapter, ``An Unexpected Correspondence,'' where the vertex count, edge count, Euler characteristic, and spectral decomposition of $K_4$ are compared to independently measured physical constants. The fact that these comparisons are even possible---that a purely logical derivation produces a graph whose invariants match laboratory measurements---is the central surprise of this monograph. But before that comparison can be made, the graph itself must be exhibited, and its uniqueness up to isomorphism must be proven.
...
%═══════════════════════════════════════════════════════════════════════════
\chapter{An Unexpected Correspondence}
\label{ch:unexpected-correspondence}
%═══════════════════════════════════════════════════════════════════════════

There is something genuinely curious about the structure we have just derived.

Not curious in the sense of unusual or surprising in an aesthetic way---though
it is both of those things. Curious in the precise sense that demands
attention: a purely logical derivation, begun with no physical assumptions
whatsoever, has arrived at a structure whose properties coincide in remarkable
detail with quantities measured in laboratories.

This coincidence may have a mundane explanation. It may not. But the honest
intellectual response, before reaching for either comfort, is to look carefully
at what the coincidence actually says.

\section{What the Graph Contains}
\label{sec:unexpected-correspondence:what-the-graph-contains}

$K_4$ has four vertices. This is not a property we imposed; it is the count that
...
The spacetime we inhabit also has four dimensions. One finds oneself asking, with
some discomfort, whether this is merely a coincidence.

$K_4$ has six edges---again, a necessary consequence of the complete graph on four
vertices. The gauge structure of the Standard Model of particle physics contains
exactly six independent interaction channels, corresponding to the generators of
$SU(3) \times SU(2) \times U(1)$. This number, too, was not placed here by intention.
...
eigenspaces have dimensions one and three. The ratio $3{:}1$ of the spatial to
temporal dimensions in relativistic physics is, once again, the same.

Each of these observations, taken alone, might be dismissed. Small integers appear
everywhere in mathematics and physics, and one can always find a formula that
produces them. The sceptical response is entirely reasonable at this stage.

What makes the situation harder to dismiss is the computation of the fine-structure
constant.

\section{The Spectral Invariant}
\label{sec:unexpected-correspondence:spectral-invariant}

The simplex complex built on $K_4$ has a computable Euler characteristic. Its
vertices, edges, triangular faces, and tetrahedral volume combine, via the
standard alternating sum, to yield
\[
  \chi(K_4) = 4 - 6 + 4 - 1 = 1.
\]
But the simplicial structure also carries a coarser invariant: the Euler
characteristic of the \emph{graph} itself, counting only the contributions from
the edge skeleton. One obtains $\chi = 7$ as the relevant invariant of the
$K_4$ simplex complex when the boundary structure is taken into account in full.
From this, together with the vertex count $V = 4$ and the simplex degree $k = 2$
(the edge), one forms the quantity
\[
  \alpha^{-1} = V^k \cdot \chi + k^2 = 4^2 \cdot 7 + 4 = 112 + 25 = 137.
\]
The measured value of the inverse fine-structure constant of electromagnetism
is $\alpha^{-1} \approx 137.036$.

This computation was not aimed at that number. No parameter was adjusted. The
formula contains nothing that refers to electromagnetism, or to any physical
theory. It is an evaluation of combinatorial invariants of the graph $K_4$.

The result is $137$.

Figure~\ref{fig:k4-ledger} gathers the ledger visually: the same tetrahedral survivor carries the vertex count, edge count, Euler data, and spectral split from which the later numerical record is assembled. The point is not ornament. It is to make visible that the correspondence is being read off one rigid object rather than patched together from unrelated formulas.
...
\begin{tikzpicture}[>=Latex, node distance=2.8cm]
  ...
  \node[concept, above=1.1cm of C] (chi) {$\chi = 7$};
  \node[derives, below=1.2cm of D] (spec) {$\mathrm{Spec}(L) = \{0,4,4,4\}$};
  \node[derives, right=1.5cm of spec] (alpha) {$\alpha^{-1} = 137$};
...
\end{tikzpicture}
\caption{The $K_4$ invariant ledger: one tetrahedral survivor carrying the counts and spectra from which the later numerical correspondences are computed.}
\label{fig:k4-ledger}
\end{figure}

\section{On the Proper Response to Such a Coincidence}
\label{sec:unexpected-correspondence:proper-response}

One well-known response to apparent numerical coincidences in physics and
mathematics is to note that, given enough freedom to choose formulas, one can
always produce a desired integer. This is true and important. It is the correct
first response.

The relevant question, then, is whether that freedom exists here. The invariants
$V$, $k$, and $\chi$ are not selected from a menu of alternatives. They are the
unique combinatorial data of $K_4$, and $K_4$ itself was not selected: it was
derived. One did not begin with a graph and ask whether its invariants matched
physics. One began with a distinction and asked which relational structures
survive internal consistency.

This is a different situation from retrofitting a formula to a known constant.
Whether it is a \emph{sufficiently} different situation to carry explanatory
weight is a question that honest inquiry cannot yet resolve. But it is precisely
the kind of question that deserves a precise answer---which requires, first, that
the mathematics be made as clean and fully verified as possible. That is what the
remainder of this book attempts.

\section{The Bridge: Why This Is Not Analogy}
\label{sec:unexpected-correspondence:the-bridge}

The skeptical reader has every right to ask: even if $K_4$ is forced, why should its invariants have anything to do with physics? Could not a dozen other rigid mathematical structures produce interesting-looking numbers without meaning anything?

The answer has a precise form, and it is not ``the numbers match.'' The bridge between the derivation and physics is structural, not numerical:

\begin{quote}
In physics, an observable is defined as a quantity invariant under the symmetry group of the theory. This is not a convention. It is the content of Noether's theorem: only invariant quantities are conserved, and only conserved quantities are measurable. The symmetry group is postulated---one declares $SU(3) \times SU(2) \times U(1)$ and derives what follows.

Here, the symmetry group is not postulated. The automorphism group $\mathrm{Aut}(K_4) \cong S_4$ is \emph{derived}: it is the group of structure-preserving maps of the unique surviving graph. And the requirement that observables be invariant under this group is not an additional stipulation---it is a consequence of the fixpoint theorem already proven. Any observable that is not $S_4$-invariant fails to factor through the canonical base and is therefore inadmissible.
\end{quote}

The bridge, stated precisely, is this: \emph{the same criterion that physics uses to separate observables from gauge artifacts---invariance under the structure group---is here forced by elimination rather than postulated as a principle.}

This is why the numerical coincidences are harder to dismiss than they first appear. The issue is not that someone found a formula producing 137. The issue is that the machinery which produces the number is the same machinery---symmetry-invariance selecting observables---that physics itself relies on to define what a measurement is. The formula is a by-product. The bridge is the mechanism.

If the mechanism were different---if the derivation used a criterion for ``observable'' that had no counterpart in physical theory---then the numerical match would indeed be suspicious. One could always ask: why this criterion and not another? But the criterion is not novel. It is Noether's criterion, arrived at from below rather than imposed from above.

This does not prove that the derivation \emph{is} physics. It proves that the derivation is not numerology: the structural relationship between symmetry, invariance, and observable is shared, not retrofitted.

\section{Why This Is Not Numerology}
\label{sec:unexpected-correspondence:not-numerology}
...
The elimination protocol in the Introduction (\S\ref{sec:introduction:elimination-protocol}) lists every step. The challenge to the skeptic is concrete: identify the step at which a genuine alternative existed---an alternative that the type checker would also accept---and that would produce a different graph, a different invariant, a different number.
...
\item \textbf{$\alpha^{-1} = 137$.} The formula $V^k \cdot \chi + k^2$ is the evaluation of $K_4$'s combinatorial invariants. The values $V=4$, $k=2$, $\chi=7$ are determined. The computation is $4^2 \cdot 7 + 4 = 137$. There is nothing to tune.
\end{enumerate}

Numerology, by contrast, has a signature: the construction is chosen \emph{after} the target is known, and the degrees of freedom are large enough that hitting the target is unsurprising. The test is simple. If the construction could equally well produce 42, or 139, or 11, by adjusting a parameter, then the result is numerological. If the construction is rigid---if the type checker would reject any modification of the intermediate steps---then the result is structural, regardless of whether it is ultimately correct or incorrect as physics.

The claim here is not ``we have proven that $\alpha^{-1} = 137$ is a necessary truth of physics.'' The claim is narrower and harder: \emph{the construction that produces 137 contains no adjustable parameter, and every step is machine-verified.} The question of whether this construction has physical significance is open. The question of whether it is numerology is closed.

Any serious critic must therefore engage at a different level. The productive questions are:

\begin{itemize}
\item Is there a hidden assumption in the starting point that smuggles in physical content?
\item Is the identification of $K_4$'s invariants with physical quantities justified by more than numerical coincidence?
\item Does the bridge (\S\ref{sec:unexpected-correspondence:the-bridge}) carry structural weight, or is it a post-hoc rationalization?
\end{itemize}

These are the right questions. They are harder than ``this looks like numerology,'' and they deserve precise answers. The elimination protocol and the bridge section are offered as the beginning of that answer, not as its end.

\section{What Follows, and Why}
\label{sec:unexpected-correspondence:what-follows-why}

To evaluate the correspondence seriously, one needs more than the graph. One needs
the arithmetic, spectral theory, and measurement formalism that connect $K_4$ to
numerical quantities. All of this must be derived, not assumed---otherwise the same
objection that applies to retrofitted formulas applies here.
...
rational approximations leave open. At each stage, the question posed is the same
one that produced $K_4$: what is the unique structure that survives the constraints
already established? Nothing is postulated. Nothing is adjusted.

At the end, all derived constants are assembled into a physics representation record
in which every field is a verified computation on $K_4$ data, and every constraint
proof closes with \texttt{refl}. This is not a rhetorical claim. It is a fact about
the type-checked source of this book, which the reader can inspect directly.

Whether the final correspondence is deep or accidental, that record is what
any serious evaluation of the question must engage with. It is offered here in
exactly that spirit.

\paragraph{Where we now stand.}
The eliminative part of the argument has closed. Observables have collapsed to canonical form, the fixpoint has been reached, and the surviving graph is $K_4$. The bridge chapter has stated why that survival is not merely a combinatorial curiosity but the hinge on which the rest of the document turns.

\paragraph{What must happen next.}
From here onward, the question is no longer which graph survives. That question is settled. The remaining task is numerical: derive the arithmetic and spectral machinery that lets the invariants of $K_4$ speak in the language of measurement.
...
The K$_4$ graph presentation proven in \hyperref[law:graph-presentation:presentation-collapse]{Law~13.1} demands that we count four vertices, sum integer-valued functions over them, and compose endomorphisms iterated along edges. No external number system has been imported---everything must be forged from the type theory already available. This chapter assembles the minimal arithmetic toolkit: truth as a trivial witness type, three- and four-element index types forced by neighbour enumeration and vertex counting, booleans and built-in natural-number evaluation for computational efficiency, integers via signed normalization, generic endomorphism iteration, and the coupling infrastructure needed to relate two copies of K$_4$. Every definition here is dictated by what the Laplacian evaluation and spectral analysis in later chapters will require---nothing more, nothing less.
...
\paragraph{Where we now stand.}
The raw arithmetic substrate is assembled. Finite indices, integers, iteration, couplings, and transport across the surviving K$_4$ cases are now available in exactly the form the graph will require. The document can now stop merely naming the surviving structures and begin imposing stable algebra on them.
...
The four combinatorial constants of the $3$-simplex---vertex count $4$, degree $3$, edge count $6$, Euler characteristic $2$---are definitional equalities, not parameters. They are recorded here to be referenced symbolically in later computations.
...
\paragraph{Where we now stand.}
Integer arithmetic is now rigid enough to support genuine spectral calculation. The K$_4$ neighbourhood structure, the Laplacian, simplex invariants, multiplication laws, and order laws have all been forced into place. The graph is no longer just a survivor; it is now an operator-bearing object.

\paragraph{What must happen next.}
But spectral data does not remain inside the integers. Ratios appear, and with them the need for quotient arithmetic, distance, and measurement. The next transition is therefore from integer combinatorics to rational evaluation.

\chapter{Rational Numbers and Measurement}
\label{ch:rational-numbers-measurement}

Eigenvalue quotients from the K$_4$ spectral analysis---ratios of integers that are not themselves integers---demand a quotient field. $\mathbb{Q}$ is defined as the forced quotient $\mathbb{Z}/\mathbb{N}^+$: an integer numerator over a positive denominator, with equality given by the cross-multiplication setoid $a \cdot d = c \cdot b$. Distance is the absolute difference of two rationals, and measurement acts on Distinctions are classified as K$_4$-maps into the two-element carrier. The simplex evaluation $\alpha^{-1} = 137$ emerges as a forced combinatorial invariant of the graph, not a fitted parameter.
...
\label{sec:rational-measurement:alpha-inverse-137}
...
\paragraph{Where we now stand.}
The spectral theory of the coupled and triple $K_4$ Laplacians is complete. Every iterated operator word collapses to one of four normal forms—identity, pure $L$-power, pure $J$-power, or zero—and the eigenvalue spectrum is exhausted by $\{0, 4, 12\}$. The survivor structure of $K_4$ has been fully extracted into operator algebra, and no spectral degree of freedom remains.

\paragraph{What must happen next.}
The spectral data is exact, but the arithmetic on which it rests—$\mathbb{Z}$ and $\mathbb{Q}$—is not yet equipped with the order and distance infrastructure that convergence arguments demand. The next chapters forge that infrastructure: additive order monotonicity, rational multiplication, $\varepsilon$-splitting, and distance laws. These are not new structure; they are the forced algebraic consequences of the integer and rational definitions already fixed. Once they are in place, the spectral data will have a metric home, and the passage to the reals will become inevitable.

\paragraph{A physics breadcrumb.}
The reader who has tracked the numbers should notice: the graph has 4 vertices and an eigenvalue ratio $3{:}1$ (three copies of eigenvalue 4, one copy of eigenvalue 0). These ratios will reappear when the derived constants are compared to independently measured quantities. The elimination is not yet complete, but the spectral fingerprint that will underlie the fine-structure computation is already fully determined.
...
\paragraph{Where we now stand.}
The rational distance function is now fully operational: triangle inequality, translation invariance, multiplicative scaling, $\varepsilon$-splitting, and distance-order interconversion are all established. Together with the additive and multiplicative order laws from the preceding chapters, $\mathbb{Q}$ is now a complete ordered field except for one property: metric completeness. Not every Cauchy sequence of rationals converges to a rational. That deficiency, once named, forces the next construction.

\paragraph{What must happen next.}
The Archimedean property of $\mathbb{Q}$ must be made explicit so that convergence estimates can quantify "how fast" sequences approach their limits. With Archimedean scaling in hand and the operator span of $K_{12}$ reduced to its canonical $(I,J)$-basis, the spectral action of the Laplacian will complete the rational infrastructure. After that, no further rational law will be needed—only the passage to the reals remains.
...
is now forced by the previously fixed sum and scaling
identities---no alternative closure rule survives.
...
the zero vector.  The final exclusion law is now packaged with no
...
\paragraph{Where we now stand.}
The rational measurement layer is complete enough to express the spectral content exactly. Quotients, distances, finite-index operators, coupled and triple K$_4$ Laplacians, word algebra, and rational order now describe the combinatorial data without leaving gaps at the level of explicit calculation. The integer $\alpha^{-1} = 137$ was already computed as a Simplex-Euler invariant in Chapter~\ref{ch:rational-numbers-measurement}; the spectral eigenvalues $\{0, 4\}$ of $K_4$ are determined; and the $3{:}1$ eigenspace split matches the dimensional signature of relativistic spacetime. These numbers emerged from the graph, not from physics. What the remaining chapters establish is that they cannot emerge from any other source.
...
\textbf{Construction:} Define $x \simeq_{\mathbb{R}} y$ iff for every $\varepsilon > 0$ there exists $N$ such that $\texttt{distℚ}(x_n, y_n) < \varepsilon$ for all $n \geq N$.  Symmetry follows from $\texttt{distℚ-sym}$.
...
\textbf{Construction:} Quarter $\varepsilon$, extract moduli independently from each summand, and drive the total distance below~$\varepsilon$ via the triangle inequality.
...
\textbf{Construction:} Define $-x$ by pointwise rational negation; define $x - y = x + (-y)$; embed $0$ and $1$ as constant sequences.

\textbf{Consequence:} The additive group structure is now complete.  No further algebraic primitive is needed until multiplication.
...
\textbf{Construction:} For each law, set the Cauchy threshold to $N = 0$ and invoke the corresponding rational identity to show that the distance between the two sides is identically zero.
...
later physical quantity depends on.  But admitting multiplication forces
...
condition at tolerance~$1$, then decompose the product distance via
the identity $fg - f'g' = f(g-g') + g'(f-f')$ and drive each summand
below~$\varepsilon/4$ using the bound and $\delta$-scaling.
...
Addition forms a group, but a group alone cannot encode physics: the coupling constants that will emerge from $K_4$ are multiplicative invariants, and the spectral identities interleave products with sums.  If multiplication on~$\mathbb{R}$ does not obey the same algebra as multiplication on~$\mathbb{Q}$, the constants lose their ring-theoretic home and the later observables cannot be defined.
...
\textbf{Construction:} For each law, set the Cauchy threshold to $N = 0$, invoke the corresponding rational identity, and conclude via \texttt{distℚ-≃0} that the distance between the two sides vanishes.
...
A field without order cannot host inequalities, and inequalities are the language of physics: every bound, every spectral gap, every convergence statement is an order statement.  But reals are equivalence classes of Cauchy sequences, and finite-prefix comparisons do not survive the passage to the quotient.  The only order that does survive is asymptotic: a comparison that holds on every tail, for every positive margin.
...
\textbf{Construction:} Define $x \leq_{\mathbb{R}} y$ iff for every $\varepsilon > 0$ there exists $N$ such that $x_n \leq y_n + \varepsilon$ for all $n \geq N$.  Define $x <_{\mathbb{R}} y$ iff there exists $\varepsilon > 0$ with the same tail comparison.
...
\textbf{Construction:} Quarter $\varepsilon_0$ into four budgets, spend two on transporting through $x \simeq x'$ and $y \simeq y'$, and retain a strictly positive remainder as the new witness.
...
\textbf{Construction:} Chain $x' \leq x$ (from $x \simeq x'$), $x \leq y$ (given), and $y \leq y'$ (from $y \simeq y'$) via transitivity.

\textbf{Consequence:} $\leq_{\mathbb{R}}$ descends to the quotient.  Together with $<_{\mathbb{R}}$, the full order structure is now representative-independent.
...
An ordered ring in which addition can reverse an inequality is useless for physics: every energy shift, every spectral translation depends on monotonicity under addition.  Adding a common term to both sides of an inequality must not destroy the comparison, or the later additive transport laws for the physical constants collapse.
...
\textbf{Construction:} Lift pointwise rational monotonicity through \texttt{+ℚ-mono-≤-right}, then use $\varepsilon$-margin associativity to place the surviving margin correctly.
...
\paragraph{Where we now stand.}
The numerical machinery is now in place. Natural numbers, integers, rationals, reals, order, distance, scaling, and spectral operators have all been forced from the surviving structure rather than imported from outside it. The document now possesses the full language required to evaluate $K_4$ quantitatively.

\paragraph{What must happen next.}
The final step is assembly. The invariants already derived must be gathered into a single verified record so that the physical correspondence can be inspected as computation rather than impression.
...
The arithmetic, spectral theory, and measurement formalism built in the preceding chapters constitute the apparatus needed to translate $K_4$'s combinatorial invariants into numbers. This part performs that translation.

Nothing new is assumed. The vertex count, edge count, Euler characteristic, and Laplacian eigenvalues of $K_4$ are assembled into a verified computation record. Each entry is a definitional equality closed by the Agda type system. The numerical values that result---including the inverse fine-structure constant $\alpha^{-1} = 137$---are consequences of the structure derived in Parts I through IV, not inputs chosen to match them.

This is the methodological boundary that matters for the numerology question. The numerical match is not being used here as evidence for the structure. The structure has already been forced by the elimination chain. What appears in this part is the evaluated consequence of that survivor. The burden is therefore not pattern recognition but invariance under the chain of eliminations that left no adjustable parameter standing.

The reader should therefore keep two claims sharply apart. The theorem-level claim is internal to the formal system: given the derivation already established, these equalities reduce and these numbers compute. The interpretation-level claim is external: some of these computed values may be read as physically meaningful because they align with measured observables. The first claim is proved here. The second is discussed here, but it is not smuggled into the proof.

Whether these numbers illuminate the structure of physics or merely reproduce it is a question the record itself cannot settle. That is an interpretive burden, not a formal one. But the record itself is exact, machine-verified, and fully open to inspection.
...
\textbf{Construction:} A three-argument \texttt{gcd-fuel} recurrence that subtracts the smaller input from the larger at each step, consuming one unit of fuel.  A \texttt{div-fuel} recurrence that counts how many times the divisor fits.
...
\textbf{Construction:} Lift rational values into $\mathit{Set}_\omega$, package their explicit quotient data, and then lift that presentation pointwise along admissible observables. Reduced witnesses are defined as positive numerator/denominator presentations together with coprimality, and uniqueness is forced by the quotient fields themselves. The same mechanism now also extracts a canonical reduction candidate from the raw fields by dividing numerator and denominator by their computed gcd.
...
\textbf{Construction:} A record \texttt{Divides⁺ d n} with field \texttt{quotient⁺} and factorization proof.  Closure under $+$, $-$, multiplication, and cancellation.
...
The factor constraints above show that divisibility by a product forces each factor to be trivial when the product divides a single copy of one factor. This is the positive-integer analogue of the familiar number-theoretic fact that if $ab \mid b$, then $a = 1$. With this in hand, the machinery can now compare quotients: if $d$ divides both $m$ and $n$ and $m \le n$, then the stored quotient for $m$ is at most the stored quotient for $n$.
...
The Euclidean gcd is now defined, but a bare recurrence carries no force.  Three properties must be forged before the gcd can serve the reduction pipeline:
...
The inductive core of the gcd engine is now established: for any fuel level sufficient to reach termination, the positive output divides both inputs. The remaining step generalises this to the canonical fuel formula $\texttt{slack} + (m + n)$, which always provides enough iterations. This is the interface consumed by the rational reduction machinery above.
...
The gcd engine and the divisibility theory are now both in place.  The canonical reduction candidate is therefore forced: given a rational $q = n/d$, compute $g = \gcd(|n|, d)$, and define the reduced numerator as $|n|/g$ and the reduced denominator as $d/g$.  The ``candidate'' is not a guess; it is the unique pair obtained by dividing out the maximal common factor.
...
\textbf{Construction:} Define \texttt{canonical-reduction-divisor} as $\texttt{mkℕ⁺}(\gcd(|n|,d) - 1)$.
Define \texttt{canonical-reduction-num} and \texttt{canonical-reduction-den} by exact division.
Prove positivity, value agreement, and divisibility.

\textbf{Consequence:} The candidate is well-defined and the gcd divides both fields.  The factorization and coprimality proofs that follow can now proceed from concrete divisibility witnesses.
...
\textbf{Construction:} Factor both fields through exact division; prove coprimality by lifting any putative common divisor back through the gcd; prove stabilization by specializing to divisor $= 1$.
...
\begin{code}
-- § The candidate numerator factorization now follows from exact division.
...
-- § The candidate denominator factorization now follows from exact division.
...
\textbf{Construction:} \texttt{ReducedRationalForm} for the nonneg sector; \texttt{SignedReducedRationalForm} as the signed generalisation; \texttt{reduced-form→signed} as the canonical embedding; and \texttt{-ℚ-resp-≃} plus \texttt{magℤ-neg-fromℕℤ} as the sign-transport lemmas.

\textbf{Consequence:} Every nonneg form lifts to a signed form.  Every negative class can now be reduced by negation transport.  The zero class is trivially nonneg.  Together, these three cases cover all of~$\mathbb{Q}$.
...
The individual components---coprimality, factorization, sign handling, nonnegative witnesses---are now all in place.  What remains is to forge them into the global theorem: every rational class, regardless of sign, admits a canonical signed reduced form.
...
\textbf{Construction:} Package the nonnegative factorization into \texttt{CanonicalReductionFactorization}, derive the quotient-level \texttt{ReducedRationalForm}, lift to \texttt{SignedReducedRationalForm} for nonneg, and handle the negative case by reducing the negation and transporting the result.

\textbf{Consequence:} \texttt{canonical-reduction-form} is total on all of~$\mathbb{Q}$: zero, positive, and negative classes.  The downstream physics chapters may now invoke reduced quotient data without case analysis.
...
-- § Under the missing gcd-factorization laws, the canonical candidate is the quotient-level reduced form.
...
-- § Every nonnegative rational class now has a canonical quotient-level reduced form.
...
The nonnegative case is now fully resolved: every nonnegative rational has a unique factorization through the canonical candidate, and that candidate yields a quotient-level reduced form. What remains is the sign barrier---negative rationals must be handled by reducing their magnitude and then transporting the result through negation.

\begin{code}
-- § Canonical reduction now yields a signed quotient-level reduced form for every rational.
...
\textbf{Construction:} Lift \texttt{RationalPresentation} and \texttt{ReducedRationalWitness} pointwise along \texttt{AdmissibleRationalObservable}, inheriting uniqueness from the class-level theorems.
...
The second kind is interpretation. Interpretation begins only when one asks whether a formally forced invariant should be read as a physical observable, a coupling, a mass ratio, or a dimensional constant. That move is not part of the theorem. It is a separate act of comparison between the formal survivor and the measured world.

\textbf{Constraint:} The numerology objection gains force precisely when theorem and interpretation are allowed to blur. If a computed invariant is described as physically significant before the formal status of the computation is fixed, the reader can rightly suspect that the target meaning guided the construction.

\textbf{Construction:} Keep the registers separate. First compute the invariant as a theorem internal to the $K_4$ record. Only afterward state the possible physical reading as an interpretation of that already-fixed value. In this chapter, proofs close in the first register; comparisons to measurement are explicitly marked as belonging to the second.

\textbf{Consequence:} The formal content of the chapter stands or falls independently of any physical interpretation. If an interpretation later fails, the theorem remains proved. If an interpretation later succeeds, it succeeds because it reads a pre-existing invariant rather than because the invariant was engineered to fit it.

%───────────────────────────────────────────────────────────────────────────
\section{Simplex invariants}
\label{sec:k4-physics:simplex-invariants}
%───────────────────────────────────────────────────────────────────────────
...
\label{sec:k4-physics:clifford-spectral-constants}
%───────────────────────────────────────────────────────────────────────────
...
\label{sec:k4-physics:aliases-cycle-counts}
%───────────────────────────────────────────────────────────────────────────
...
Every combinatorial constant of $K_4$ is now fixed by definitional equality: vertex count, edge count, face count, degree, Euler characteristic, spectral width, and cycle data. The ledger is sealed. What remains is to show that these constants determine a unique formal representation record---that no second assignment of fields could survive the same constraints.

\paragraph{The numbers so far.}
At the theorem level, $K_4$ has produced $V = 4$, $E = 6$, an eigenvalue ratio $3{:}1$, and the combinatorial invariant $\alpha^{-1} = 137$. None of these numbers were adjusted. Each is a definitional equality closed by the Agda type checker. At the interpretation level, one may compare these values with spacetime dimension, gauge-sector counts, spatial-to-temporal split, and the inverse fine-structure constant. The next chapter addresses the last formal question first: is the assignment of fields to these invariants itself unique, or could a second admissible representation survive the same graph?
...
A physics representation that floats freely above its graph would reintroduce contingency at the worst possible moment: after every graph invariant has been locked down.  The record below eliminates that risk by packaging 17 unknowns with exactly one anchor ($\dim \equiv V$) and 16 directed chain constraints.  Once the anchor connects to $K_4$, every remaining field is forced to exactly one value through a rigid directed chain: $\dim$ fixes $n\text{-spatial}$, which fixes $n\text{-temporal}$, which fixes every subsequent field.

\textbf{Constraint:} Every physical field must be uniquely determined by the $K_4$ anchor; no field may carry independent freedom.

\textbf{Construction:} Package 17 fields with 1~anchor ($\dim \equiv \texttt{vertexCountK4}$) and 16~chained equalities.  Each equality pins one field to a formula in previously fixed fields.
...
\textbf{Construction:} Assign each field its forced value (4, 3, 1, 6, 4, 2, 137, \ldots) and close every constraint proof by \texttt{refl}.
...
\textbf{Construction:} Start from $\dim \equiv 4$ (the anchor).  Derive $n\text{-spatial} \equiv 3$ via \texttt{cg-deg}, then $n\text{-temporal} \equiv 1$ via \texttt{cg-temporal}, and so on through all 17~fields.
...
Existence alone leaves a lethal gap: if two distinct representations of $K_4$ could coexist, the physical constants would depend on which representation was chosen, and no experiment could decide between them.  Uniqueness closes that gap.  Since \texttt{ForcedValues} pins every field of every admissible representation to the same normal form, any two representations must agree field by field.  Singletonhood is therefore proved, not declared.
...
\textbf{Construction:} For each field, chain $F_1.\text{field-is-}k$ with $\text{sym}\;F_2.\text{field-is-}k$ to obtain $r_1.\text{field} \equiv r_2.\text{field}$.

\textbf{Consequence:} The physics representation of $K_4$ is a singleton.  No alternative survives.
...
module CrossConstraints where
  ...
  cross-spatial-is-gen : n-spatial canonical-rep ≡ n-gen canonical-rep
  ...
  cross-baryon-is-gauge : baryon-den canonical-rep ≡ gauge-rank canonical-rep
...
Every \texttt{K4PhysRep} field is now forced directly from named $K_4$ invariants established earlier. Bare numerals disappear from the assignment layer, and each proof closes by \texttt{refl} because the chain formulas are exactly the definitions of the named invariants they invoke.
...
\label{sec:k4-representation:physical-quantities-computable-invariants}
%───────────────────────────────────────────────────────────────────────────

The remaining formal gap between graph invariants and the representation record is eliminated here. Each represented quantity is forced to be a computable function of $K_4$ data, so the internal identification is no longer rhetorical: the output of the graph computation is exactly the value carried by the representation chain. Whether a given represented quantity should also be read as a physical observable remains a separate interpretive question.

\subsection{Representation Completeness}
\label{sec:k4-representation:representation-completeness}

The record built below is already rigid once one enters it. But the stronger pressure comes from outside. If there were two admissible ways for physics to sit on the same $K_4$ invariant spine, then the constants would no longer be observables forced by structure; they would be artifacts of representational wording. That would collapse the claim at the final step. The graph would still survive, but the bridge into measurement would fork, and every numerical coincidence could be dismissed as notation. This fork must be shattered.

\textbf{Constraint:} Any admissible physical representation of $K_4$ must preserve the named invariant chain already fixed: vertex count, edge count, face count, Euler characteristic, spectral data, and the derived observables assembled from them. No representational wrapper may introduce additional degrees of freedom while claiming to describe the same structure.

\textbf{Construction:} The present chapter proves singletonhood for the internal record \texttt{K4PhysRep}.
The external completeness theorem is now provided below as \texttt{RepresentationComplete}: for any admissible $K_4$ representation~$r$, every observable field agrees with the canonical record.
\[
\mathrm{RepresentationComplete} :
\bigl(\text{admissible representation of }K_4\bigr)
\to
\bigl(\text{canonical }K4PhysRep\bigr),
\]
...
\textbf{Consequence:} With the closure theorem \texttt{RepresentationComplete} now explicit, the formal bridge no longer says merely that one chosen representation works. It says that no rival admissible representation survives. Any later physical reading must therefore attach to a unique formal record rather than to a convenient encoding chosen after the fact.

\begin{code}
-- § RepresentationComplete: every K4PhysRep agrees with canonical-rep on all values.
module RepresentationComplete (r : K4PhysRep) where
  ...
  val-gauge      : gauge-rank r   ≡ gauge-rank canonical-rep
  val-gauge      = F.gauge-is-6
  ...
  val-coupling   : coupling-inv r ≡ coupling-inv canonical-rep
  val-coupling   = F.coupling-is-137

  val-gen        : n-gen r        ≡ n-gen canonical-rep
  val-gen        = F.gen-is-3

  val-spinor     : spinor-dim r   ≡ spinor-dim canonical-rep
  val-spinor     = F.spinor-is-16
  ...
  val-bh         : bh-norm r      ≡ bh-norm canonical-rep
  val-bh         = F.bh-is-4

  val-uv         : uv-cutoff r    ≡ uv-cutoff canonical-rep
  val-uv         = F.uv-is-1

  val-loop       : min-loop r     ≡ min-loop canonical-rep
  val-loop       = F.loop-is-1

  val-baryon-num : baryon-num r   ≡ baryon-num canonical-rep
  val-baryon-num = F.baryon-num-is-1

  val-baryon-den : baryon-den r   ≡ baryon-den canonical-rep
  val-baryon-den = F.baryon-den-is-6
\end{code}
...
Once the local observables are fixed, the rest of the physics-facing quantities are short global computations. Splitting them into a second block mirrors the earlier chapters, where constructors come first and packaged consequences follow afterward.
...
The global layer can no longer remain an unrestricted numeric container. If every natural number counted as a global observable merely because it was written down after the graph was fixed, descriptive freedom would return through naming. The admissibility filter therefore has to close the space itself. Only four primitive global classes survive: boundary, correlator, observer, and channels. Everything else in the later physics discussion is either a value of this basis or a derived ledger invariant, not a fifth admissible global observable.
...
\textbf{Construction:} The admissible global sector is represented by a finite four-code basis, together with its canonical evaluation map into the already forced graph quantities.

\textbf{Consequence:} Global admissibility is now an elimination statement rather than a naming convention. The space of primitive global observables is closed, finite, and exhausted by the four surviving codes.
...
\subsubsection*{Threshold To Physics}

At this point the discrete chain has already delivered its verdict. The carrier has been forced into a two-class distinction. The two-class distinction has been forced into the unique $K_4$ graph. The graph has been forced into a rigid invariant ledger. And the observable principle has stripped away the last semantic slack by ruling out every quantity that depends on presentation, labeling, or proof artefacts.

What survives this pressure is no longer a loose collection of combinatorial curiosities. It is a closed discrete record: $V = 4$, $E = 6$, $d = 3$, $\chi = 2$, $\kappa = 8$, the Clifford count $16$, the compactification stratum $17$, and the rigid coupling value $\alpha^{-1} = 137$. Nothing in this ledger has been fitted. Every entry is already the survivor of an elimination chain.

This is the last point at which the work is purely discrete. The next question cannot be whether different integers would do just as well. That question has already collapsed at the theorem level. The only live question is harsher: when experiment reports decimal structure rather than bare integers, does the continuum introduce freedom, or does it merely expose the next forced quotient of the same graph?

\textbf{Constraint:} Before physics begins, the discrete stage must already be closed. If any invariant or observable were still negotiable here, every later numerical match would be contaminated by hidden freedom.

\textbf{Construction:} The preceding parts force exactly two ingredients: the invariant ledger of $K_4$, and the admissibility criterion for observables on that ledger.

\textbf{Consequence:} The physics chapter does not begin by choosing formulas. It begins with a sealed discrete record and asks whether the passage to measured continuum values is itself forced.
...
The discrete record is closed, but experiment is not reported as a ledger of bare integers. Physics is measured in a smooth continuum. The proton-to-electron mass ratio is not $1836$ but $1836.15267$, and the fine-structure constant is not $1/137$ but $1/137.036$. The question is therefore no longer which discrete invariants survive. That question is settled. The sharper question is whether the decimal structure is an independent source of freedom, or the next forced quotient of the same $K_4$ data.

The chapter proceeds in three stages, each with a distinct epistemic status:

\begin{enumerate}
\item \textbf{Primary graph invariants.} The raw data of $K_4$---$V$, $E$, $d$, $\chi$, the spectrum, $\kappa$, and $\mathrm{Aut}(K_4) \cong S_4$---are forced by the classification theorems of the preceding parts. These are the hardest results in the chain: no freedom remains.

\item \textbf{Forced observables.} Not every function of these invariants is physically admissible. Part~IV established that only observables which are presentation-independent, automorphism-invariant, and canonically factorable qualify. At the primitive global level, this admissible sector collapses to four classes only: boundary, correlator, observer, and channels. This chapter then evaluates and combines those primitive observables with the rigid ledger invariants of $K_4$ to classify the derived integers $137$, $1836$, $11$, $72$, and $576$.

\item \textbf{Canonical correction.} The discrete-to-continuum transition is not a new source of freedom but a canonical quotient: the forced loop observable supplies the numerator; the canonical interaction volume at each energy scale supplies the denominator. The resulting rational corrections---$11/72$, $11/576$---are the unique bridge between the discrete graph and the measured continuum.
\end{enumerate}

If any of these stages introduced a free parameter, the chain would break. The integer values would become numerological coincidences and the decimal places fitted afterthoughts. The strength of the argument is that no parameter enters at any stage: primary invariants are forced, observables are classified, and the correction is canonical. Interpretation enters only after those three theorem-level stages are closed.

%───────────────────────────────────────────────────────────────────────────
\section{The admissibility filter}
\label{sec:discrete-continuum:admissibility-filter}
%───────────────────────────────────────────────────────────────────────────

Part~IV already shattered the idea that every graph function deserves physical status. Most quantities collapse at the threshold. They depend on labeling, on presentation, or on proof packaging. If that noise were allowed to cross into the physics chapter, every later number would lose its force. Nature therefore permits only those quantities that survive a stricter tribunal: they must ignore presentation, survive the full symmetry group, and descend through the unique canonical observable map.

Part~IV proved the Observable Factorization Theorem (\hyperref[law:observables:factor-through-base]{Laws~9.3--9.4}): every admissible observable on the indexed drift ledger factors through a unique base computation, and that factorization is unique up to canonical identification. What counts as a ``physical quantity'' is therefore not a matter of definition but of elimination:

\begin{enumerate}
\item \textbf{Presentation invariance.} The observable must not depend on which labeling of the carrier is chosen. Any two presentations that are total, sound, and injective yield the same value (\hyperref[law:classification:distinction-to-class]{Law~1.17}).

\item \textbf{Automorphism invariance.} The value must be fixed under the full automorphism group of the underlying structure. For $K_4$, that group is $\mathrm{Aut}(K_4) \cong S_4$: only $S_4$-invariant combinations of the graph data qualify.

\item \textbf{Canonical factorization.} The observable must factor through the canonical base computation $\texttt{canonObs}$ (\hyperref[law:observables:canonObs-sound]{Law~9.10}). Observables that fail to factor are proof-dependent artefacts, not structural quantities.
\end{enumerate}

The integers derived below are not ``nice numbers from a graph.'' At the primitive global level, only four observable classes survive at all; everything else entering the physics chapter is obtained by evaluating those classes, the local observable layer, and the already forced ledger invariants on the canonical object $K_4$. The admissibility filter therefore converts the question ``why this number?'' into the sharper question ``which surviving observable class or forced invariant chain leaves it behind?'' The answer is that elimination leaves no room for a second survivor.

\textbf{Constraint:} A quantity can enter physics only if it is invariant under presentation, invariant under $\mathrm{Aut}(K_4)$, and canonically factored through the unique observable base. Any weaker criterion would let proof artefacts masquerade as observables.

\textbf{Construction:} The observable factorization theorem supplies exactly one admissibility filter: presentation invariance, automorphism invariance, and canonical factorization. On the fixed $K_4$ ledger this filter leaves a finite primitive observable basis, and the later numerical quantities arise only as evaluations or derived combinations of that basis with already forced invariants.

\textbf{Consequence:} The numbers carried into the loop and mass sectors are not selected formulas. Their primitive observable inputs are already fixed, and every later derived value is constrained by that fixed basis together with the invariant ledger, so excluded alternatives collapse before any numerical comparison with experiment begins.
...
%───────────────────────────────────────────────────────────────────────────
\section{Tree-level mass formulae}
\label{sec:discrete-continuum:tree-level-mass-formulae}
%───────────────────────────────────────────────────────────────────────────

Before any correction can be applied, the bare values must exist. They are not hypotheses; they are computable functions of the invariants already fixed.

The proton-to-electron mass ratio is built from the Euler characteristic $\chi$, the vertex degree $d$, and the Fermat stratum $F_2$. Two combinatorially distinct decompositions yield the same integer, locked together by the identity $\chi \cdot d = E$.

A direct question must be faced: why the product $\chi^2 \cdot d^3 \cdot F_2$ and not, say, $V^2 \times E \times d = 288$? The answer is not immediate logical necessity but classification: $1836 = 2^2 \times 3^3 \times 17$, and among the $K_4$ invariants the factor $17 = F_2$ is prime and appears nowhere else. The remaining factor $108 = \chi^2 d^3 = E^2 d$ admits exactly two monomial factorizations over $\{V, E, d, \chi\}$, both of minimal degree. Their agreement ($\chi^2 d^3 = E^2 d$ via the identity $\chi d = E$) is a non-trivial structural constraint. Whether this classification constitutes \emph{necessity} or merely \emph{distinction} is addressed in the classification section below.
...
%───────────────────────────────────────────────────────────────────────────
\section{The universal loop numerator}
\label{sec:discrete-continuum:universal-loop-numerator}
%───────────────────────────────────────────────────────────────────────────

When a discrete graph is projected into a continuum, interactions that were sharp graph traversals become smeared across the embedding space. The smearing generates corrections. The question is not ``what number shall we put in the numerator?'' but ``which observable is forced by the admissibility filter in the loop sector?''

On $K_4$, the primitive structural parameters for a graph interaction are the edge count ($E = 6$), the vertex degree ($d = 3$), and the Euler characteristic ($\chi = 2$). A loop observable is a $\{0,1\}$-linear combination of these---a sum of a subset. The admissibility constraints are:

\begin{enumerate}
\item \textbf{Irreducibility.} The correction fraction $N/D$ must be irreducible: $\gcd(N,\, V \!\cdot\! E \!\cdot\! d) = 1$. Common factors would indicate that the observable double-counts structure already present in the interaction volume.

\item \textbf{Completeness.} The observable must use the full irreducible loop basis, not a proper subset. This requirement is not an additional axiom: it is derived below from the $K_4$ identity $\chi \cdot d = E$ and the role of $E$ in the interaction volume.

\item \textbf{Cross-scale consistency.} The same numerator must appear at every energy scale. If different scales required different numerators, the loop observable would depend on the embedding, not on the graph.
\end{enumerate}

The classification section below proves exhaustively that among all $\{0,1\}$-linear combinations of $\{E, d, \chi\}$, exactly two are coprime to the QCD denominator $72$: the partial sum $d + \chi = 5$ and the full sum $E + d + \chi = 11$. The partial sum fails completeness (it omits the edge sector). The full sum is the unique admissible observable. Cross-scale consistency then provides a confirmation rather than an additional elimination: the partial $d + \chi = 5$ also happens to be coprime to the EW denominator $576$, so the filters that eliminate it are irreducibility at QCD scale (passing) and completeness (failing). Only $11$ passes all three.

\textbf{Constraint:} The loop observable must be admissible: irreducible, complete, and cross-scale consistent.

\textbf{Construction:} $E + d + \chi = 6 + 3 + 2 = 11$ is the unique $\{0,1\}$-linear combination satisfying all three constraints. The elimination sequence is: Filter~1 reduces 8 candidates to 2; Filter~2 eliminates the incomplete partial sum; Filter~3 confirms the result.

\textbf{Consequence:} $11$ is not proposed. It is forced by elimination: every alternative fails completeness or irreducibility.

\subsubsection*{Candidate space and denominators}
...
\subsubsection*{Exhaustive candidate enumeration}

The search space is finite, so the argument is clearer when it proceeds in the same order as the earlier census sections: enumerate first, then eliminate by filters.
...
\subsubsection*{Filter 1: irreducibility}

This first filter removes every candidate that shares a factor with the QCD interaction volume. Only the two coprime survivors are allowed to reach the structural test.
...
\subsubsection*{Filter 2: completeness}

The second filter now has an explicit internal shape: first the direct witness that $d + \chi$ omits the edge sector, then the number-theoretic argument showing that the omitted sector cannot be reconstructed from the partial sum.
...
\subsubsection*{Derivation of the completeness criterion}

The completeness requirement is not an axiom imposed from outside. It follows from three structural properties already established:

\begin{enumerate}
\item \textbf{Multiplicative identity.} The three loop basis elements satisfy $\chi \cdot d = E$ (\texttt{law-chi-times-d-is-E}). This is the unique non-trivial multiplicative relation among $\{E, d, \chi\}$: the edge count is the product of the Euler characteristic and the degree.

\item \textbf{Denominator factorization.} The interaction volume $D = V \cdot E \cdot d = 72$ factors through $E$. Substituting $E = \chi d$ yields $D = V \cdot \chi \cdot d^2$: the denominator already encodes both $\chi$ and $d$ multiplicatively, through the edge channel.

\item \textbf{Sum--product coprimality.} The partial sum $d + \chi = 5$ and the product $\chi \cdot d = E = 6$ are coprime: $\gcd(5, 6) = 1$ (\texttt{law-dchi-escapes-stratum}). Since $\chi$ and $d$ are themselves coprime ($\gcd(2, 3) = 1$), their sum and product share no factor---a number-theoretic necessity for coprime inputs.
\end{enumerate}

The consequence: the partial numerator $d + \chi$ provides the \emph{additive} combination of $\chi$ and $d$, but the denominator encodes the \emph{multiplicative} combination $\chi \cdot d = E$. These two projections of the same generators are coprime and therefore structurally disconnected. A correction fraction with $d + \chi$ in the numerator and $\chi \cdot d$ in the denominator treats two multiplicatively linked channels as if they were independent---the edge contribution to the volume has no counterpart in the numerator.

Including $E$ directly restores the connection: the full sum $E + d + \chi = 11$ references all three generators and its coprimality with $D = 72$ is verified. The completeness criterion is thus a consequence of the $K_4$ identity $\chi \cdot d = E$ together with the irreducibility constraint.

\subsubsection*{Formal completeness witness}
...
\subsubsection*{Filter 3 and final packaging}

The last stage does not introduce a new eliminator. It confirms that the same numerator survives when the denominator is enlarged by $\kappa$, and then packages the elimination result into a single record.
...
%───────────────────────────────────────────────────────────────────────────
\section{Canonical interaction volumes}
\label{sec:discrete-continuum:canonical-interaction-volumes}
%───────────────────────────────────────────────────────────────────────────

The forced observable provides the numerator. The denominator is not chosen---it is the canonical interaction volume at the relevant energy scale: the product of all independent graph coordinates that participate in the smearing.

At the QCD (hadron) scale, the interaction volume is the product of the three vertex-level graph coordinates: $V \times E \times d = 4 \times 6 \times 3 = 72$. Each factor is a primary $K_4$ invariant; their product counts the independent local configurations of the graph.

At the electroweak scale, the spectral width $\kappa$ opens an additional degree of freedom. The interaction volume becomes $V \times E \times d \times \kappa = 72 \times 8 = 576$. The factor $\kappa = 2(d{+}1) = 8$ is the same spectral constant already fixed in the preceding chapter. The scale ratio between regimes is a graph invariant, not a parameter.

\textbf{Constraint:} The denominator at each scale must be the canonical product of all graph coordinates active at that scale. No subset, no superset.

\textbf{Construction:} QCD volume $= V \cdot E \cdot d = 72$. Electroweak volume $= V \cdot E \cdot d \cdot \kappa = 576$. Their ratio is $\kappa = 8$.

\textbf{Consequence:} The denominators are determined by the same forcing principle as the numerator. They are not fit parameters but canonical products of forced invariants. The QCD and electroweak scales are related by $\kappa$, which also governs the spectral width and the CHSH bound---a single invariant linking three independent roles.
...
%───────────────────────────────────────────────────────────────────────────
\section{Proton mass correction}
\label{sec:discrete-continuum:proton-mass-correction}
%───────────────────────────────────────────────────────────────────────────

The measured proton-to-electron mass ratio is $1836.15267$ (CODATA 2022). The bare $K_4$ value is $1836$. The fractional part $0.15267$ is the QCD-scale correction forced by the loop observable:

$$
\text{correction} = \frac{\text{forced numerator}}{\text{QCD volume}} = \frac{E + d + \chi}{V \times E \times d} = \frac{11}{72} = 0.1527\overline{7}
$$

No parameter is chosen. The numerator is forced by elimination. The denominator is the canonical QCD interaction volume. The quotient is the canonical correction.

\textbf{Constraint:} The correction must use the QCD-scale volume (no spectral width factor $\kappa$, since the proton is a hadron).

\textbf{Construction:} $11/72 = 0.15277\ldots$ The corrected value is $1836 + 11/72 = 1836.1527\overline{7}$.

\textbf{Consequence:} The measured value $1836.15267$ agrees to six significant figures. The residual $|0.15278 - 0.15267| = 0.00011$ is of order $1/576$---exactly the next-order correction from the electroweak volume.
...
%───────────────────────────────────────────────────────────────────────────
\section{Electroweak correction}
\label{sec:discrete-continuum:electroweak-correction}
%───────────────────────────────────────────────────────────────────────────

The Weinberg mixing angle defines the ratio of electroweak coupling strengths. The tree-level value from $K_4$ is $\sin^2\theta_W = \chi/\kappa = 2/8 = 0.25$. The measured value is $\sin^2\theta_W = 0.2312$ (PDG 2024).

The correction is the electroweak-scale application of the same forced observable:

$$
\delta_{\text{EW}} = \frac{\text{forced numerator}}{\text{EW volume}} = \frac{E + d + \chi}{V \times E \times d \times \kappa} = \frac{11}{576} \approx 0.0191
$$

The corrected value is $0.25 - 11/576 = 0.2309$, within $0.1\%$ of the measured value.

\textbf{Constraint:} The electroweak correction must use the full canonical interaction volume including the spectral width $\kappa$.

\textbf{Construction:} $11/576 = 0.01909\overline{72}$. The corrected Weinberg angle is $\chi/\kappa - 11/576 = 2/8 - 11/576$.

\textbf{Consequence:} The same forced observable that corrects the proton mass also corrects the Weinberg angle. Only the canonical volume changes, by the factor $\kappa$. This is not cross-validation in the sense of an independent check: it is a structural consequence. The observable is forced. The volumes are canonical. The same quotient must appear at every scale---and it does.
...
\label{sec:discrete-continuum:correction-chain}
%───────────────────────────────────────────────────────────────────────────

The corrections are not added by hand. They arise from a chain of canonical maps, each forced by the elimination of alternatives:

\begin{enumerate}
\item \textbf{Graph.} $K_4$ is the unique fixpoint of the classification-and-elimination process (Parts~I--III). No other graph passes the filter.

\item \textbf{Observable.} Among all functions on the drift ledger, only those that are presentation-independent, automorphism-invariant, and base-factorable qualify as admissible observables (Part~IV). Every admissible observable reduces to its canonical form via $\texttt{canonObs}$. In the concrete $K_4$ physics layer, the primitive global sector then collapses further to four classes only: boundary, correlator, observer, and channels.

\item \textbf{Loop observable.} Among all $\{0,1\}$-linear loop observables over $\{E, d, \chi\}$, exactly one satisfies irreducibility, completeness, and cross-scale consistency: $E + d + \chi = 11$. This is not a fifth primitive global observable but a uniquely surviving derived observable built from the forced invariant ledger. Every alternative is eliminated.

\item \textbf{Canonical volume.} The interaction volume at each scale is the product of all active graph coordinates: $V \!\cdot\! E \!\cdot\! d = 72$ at QCD, $V \!\cdot\! E \!\cdot\! d \!\cdot\! \kappa = 576$ at the electroweak scale. These are derived ledger volumes, not additional primitive observables. No subset and no superset gives a canonically complete volume.

\item \textbf{Correction.} The discrete-to-continuum bridge is the canonical quotient: forced numerator / canonical volume. This is the unique map from the discrete graph to the rational correction, and it uses only forced ingredients at every step.
\end{enumerate}

Each link in this chain is a canonical map. No link introduces a free parameter, and no step quietly enlarges the primitive observable basis. The chain is therefore itself canonical---and the rational corrections are its output. The next theorem does not add a new ingredient. It packages this already established chain into a single uniqueness statement about the discrete--continuum passage itself.

Figure~\ref{fig:observable-classes-to-corrections} pulls the same chain slightly closer to the eye of the physicist. The four primitive observable classes are not yet the corrections. They are the knobs that survive after every fake knob has been removed. Once those four remain, the loop numerator and the channel volume are no longer adjustable pieces. They lock together and the two correction fractions fall out.

\begin{figure}[h]
\centering
\begin{tikzpicture}[>=Latex, node distance=2.5cm]
  \node[concept] (b) {boundary};
  \node[concept, right=1.7cm of b] (c) {correlator};
  \node[concept, right=1.7cm of c] (o) {observer};
  \node[concept, right=1.7cm of o] (ch) {channels};

  \node[derives, below=1.8cm of $(b)!0.5!(c)$] (num) {loop numerator\\$11$};
  \node[derives, below=1.8cm of $(o)!0.5!(ch)$] (vol) {canonical volume\\$72,\;576$};

  \node[concept, below=2.1cm of $(num)!0.5!(vol)$] (corr) {corrections\\$11/72,\;11/576$};

  \draw[flow] (b.south) -- ($(num.north)+(-0.55,0)$);
  \draw[flow] (c.south) -- ($(num.north)+(0.55,0)$);
  \draw[flow] (o.south) -- ($(vol.north)+(-0.55,0)$);
  \draw[flow] (ch.south) -- ($(vol.north)+(0.55,0)$);
  \draw[flow] (num) -- node[label, left] {forced quotient} (corr);
  \draw[flow] (vol) -- node[label, right] {forced quotient} (corr);
\end{tikzpicture}
\caption{From primitive observable classes to continuum corrections. Once the surviving knobs are fixed, the correction fractions are no longer tuned but read off.}
\label{fig:observable-classes-to-corrections}
\end{figure}

\subsubsection*{Admissible real-valued observables and quotient presentation}

The continuum cannot be allowed to enter as an anonymous decimal haze. Part~IV already forced admissible observables to collapse to canonical base content. Part~V now also forces the rational quotient data to remain globally visible: every admissible rational observable carries unique pointwise quotient fields, and any reduced witness is unique whenever it exists. If these ingredients are not explicitly tied together here, the reader still sees only the final fractions and is left to reconstruct the closure by hand.

This chapter therefore no longer has to invent a quotient theory locally. It only fixes the real-valued closure of the already forced rational core. A bridge observable is not an arbitrary function into $\mathbb{R}$. It is a real quantity whose surviving content already comes from an admissible rational core and reaches the continuum only through the forced closure map $\mathbb{Q} \to \mathbb{R}$. The only extra pressure specific to physics is irreducibility: common factors would mean that the correction is still repeating structure already counted in the canonical volume.

\textbf{Constraint:} A continuum quantity may not introduce fresh observational freedom beyond the admissible and canonically factored content already surviving on the discrete side. If it did, the bridge would cease to be a closure and become a new source of parameters.

\textbf{Construction:} Reuse the global class of admissible rational observables from Part~V, add the corresponding real-valued codomain, and admit only those real observables that are exactly the $\mathbb{Q}$-to-$\mathbb{R}$ closure of an admissible rational core. Quotient presentation is therefore inherited rather than postulated; irreducibility is then imposed where a correction fraction is claimed.

\textbf{Consequence:} The discrete--continuum map no longer appears as a jump from integers to decimals. It becomes locally visible as a forced chain: admissible observable, canonical rational quotient, irreducibility filter, real closure. The correction theorem below can then speak about uniqueness without hiding where numerator and denominator come from.
...
\subsubsection*{Theorem: Uniqueness of the Discrete--Continuum Map}

At this point the bridge can no longer be described merely as a successful recipe. If the passage from the discrete ledger to the continuum correction admitted two inequivalent realizations, the entire chapter would lose force at the last moment. The graph could still be rigid, the admissibility filter could still be sharp, and yet the measured correction would depend on representational wording. That fork is exactly what the preceding elimination chain has already destroyed.

Nothing new is introduced here. The theorem only hardens what the chapter has already forced in pieces. The admissible observable content is first collapsed to a canonical rational core. That core already carries quotient fields. In the correction sector the numerator is then classified uniquely, the scale fixes the canonical denominator uniquely, and irreducibility blocks any common-factor slack. Once those ingredients are fixed, the real closure cannot branch without reintroducing a parameter the chapter has already eliminated.

\textbf{Constraint:} A discrete--continuum map is admissible only if it uses admissible observable content, passes through the canonical rational quotient layer, and then closes into $\mathbb{R}$ without adding new structure. In the correction sector this means: unique surviving loop numerator, unique canonical interaction volume, and irreducibility of the resulting fraction.

\textbf{Construction:} Package the bridge as a scale-indexed map with two cases only: the QCD correction $11/72$ and the electroweak correction $11/576$. The numerator is shared because the loop observable is unique. The denominator changes only by the already forced spectral factor $\kappa$. The quotient fields are then fixed by the rational values themselves, and irreducibility is verified inside the correction sector.

The canonical reduced witnesses below make this visible in the sharpest local form the chapter currently supports: each correction is not only a rational quotient, but a quotient already written in its surviving reduced presentation.

\textbf{Consequence:} The discrete--continuum passage does not fork. At every scale treated in this chapter, the bridge is fixed by the unique numerator together with the canonical volume, and the real layer contributes only closure, not new freedom. The specific corrections $11/72$ and $11/576$ are therefore not chosen outputs. They are the unique irreducible rational quotients forced by the discrete record at the treated scales.

\phantomsection\label{law:discrete-continuum:map-unique}
...
The theorem and the following diagram say the same thing at two different levels: the code packages the bridge as a unique scale-indexed map, and the diagram makes visible which forced inputs leave that map with no room to branch.

The following diagram summarizes the five links:

\begin{figure}[h]
\centering
\begin{tikzpicture}[>=Latex, node distance=2.8cm]
  \node[concept] (G)  {$K_4$\\{\small graph}};
  \node[concept, right of=G]  (O) {obs\\{\small admissible}};
  \node[concept, right of=O]  (L) {$11$\\{\small loop}};
  \node[concept, right of=L]  (V) {$72,\,576$\\{\small volume}};
  \node[derives, right of=V]  (C) {$\tfrac{11}{72},\,\tfrac{11}{576}$\\{\small correction}};
  \draw[flow] (G) -- node[label, above] {Parts I--III} (O);
  \draw[flow] (O) -- node[label, above] {Part IV} (L);
  \draw[flow] (L) -- node[label, above] {$V\!\cdot\!E\!\cdot\!d$} (V);
  \draw[flow] (V) -- node[label, above] {quotient} (C);
\end{tikzpicture}
\caption{The correction chain.  Each arrow is a canonical map forced by elimination.}
\label{fig:correction-chain}
\end{figure}

\begin{center}
\begin{tabular}{llccc}
\toprule
\textbf{Derived quantity} & \textbf{Bare} & \textbf{Numerator} & \textbf{Derived volume} & \textbf{Correction} \\
\midrule
Proton mass ratio & $1836$  & $11$ & $72 = V \!\cdot\! E \!\cdot\! d$ & $+11/72$ \\
Weinberg angle    & $0.25$  & $11$ & $576 = V \!\cdot\! E \!\cdot\! d \!\cdot\! \kappa$ & $-11/576$ \\
RG slope          & ---     & $1$  & $274 = 2\alpha^{-1}$ & $1/274$ \\
\bottomrule
\end{tabular}
\end{center}

The table now reads as a corollary of the theorem rather than as an isolated summary. Proton and electroweak corrections share the same forced loop numerator~$11$ because the loop sector survives elimination only once and is then evaluated at two canonical volumes. The RG slope belongs to a different derived channel: its numerator is fixed at~$1$, while its denominator is forced by the already classified coupling value $\alpha^{-1} = 137$. In each case, the continuum correction is downstream of already fixed discrete data.

%───────────────────────────────────────────────────────────────────────────
\section{Classification of Derived Sectors}
\label{sec:discrete-continuum:classification-derived-sectors}
%───────────────────────────────────────────────────────────────────────────

The preceding sections fixed the admissibility filter and the primitive observable basis. But the physics chapter does not end at that basis. It must still show how the derived sectors built from the ledger survive elimination. This section performs that harsher census: not of new primitive observables, but of the derived candidate spaces for coupling, mass, loop numerator, and correction volume. In each case the task is the same. A finite sector is exposed, weaker survivors are shattered, and exactly one derived quantity remains.

\subsection{Classification of \texorpdfstring{$\alpha^{-1} = 137$}{α⁻¹ = 137}}
\label{sec:discrete-continuum:classification-alpha-137}

The value $\alpha^{-1} = 137$ does not enter here as a chosen formula. By the stage at which it is classified, the construction is already closed in the only places where hidden freedom could have entered: the graph is fixed as $K_4$, the invariant ledger $(V,E,d,\chi,\kappa)$ is fixed, and the admissibility filter has already eliminated presentation-dependent, non-canonical, and proof-sensitive quantities. The remaining task is therefore not to pick a coupling value, but to expose the unique survivor in the derived coupling sector.

\textbf{Constraint:} The coupling value must remain inside the admissible derived sector determined by the earlier filter. Any candidate must be invariant under $\mathrm{Aut}(K_4)$, factor through the canonical invariant basis already fixed, and stay within the graph-forced local complexity bound. If one relaxes any of these conditions, the value is no longer derived from the surviving structure but from reintroduced representational slack.

\textbf{Construction:} Classification first eliminates every single monomial in the invariant basis as a possible source of $137$, then classifies all degree-$\le 3$ admissible two-term completions. Exactly one decomposition survives: $128 + 9 = V^3\chi + d^2 = V^2\kappa + d^2$.

\textbf{Consequence:} The value is not fitted to experiment and not selected by taste. It is the unique survivor of the admissibility constraints in the bounded derived coupling sector. Any alternative value would require either non-invariant structure, non-canonical observables, or restored representational freedom, and each of those has already been eliminated upstream.

The coupling constant $\alpha^{-1} = 137$ is prime. No single monomial in the $K_4$ invariants $\{V{=}4,\, E{=}6,\, d{=}3,\, \chi{=}2,\, \kappa{=}8\}$ of degree $\le 3$ evaluates to~$137$, because all such monomials factor through $2$, $3$, or both. The value is therefore not a coincidence of a single structural factor; it requires at least two terms.

Among all pairs $(M_1, M_2)$ where each $M_i$ is a monomial of degree $\le 3$ in the $K_4$ invariants, the decomposition $M_1 + M_2 = 137$ has exactly one solution: $128 + 9$. The first term $128 = V^3 \chi = V^2 \kappa$ combines spectral scaling with topology; the second term $9 = d^2$ is the geometric square of the vertex degree. No other pair of degree-$\le 3$ monomials sums to $137$.

\subsubsection*{Prime exclusion and the surviving pair}
...
\subsubsection*{Degree bound and two-term necessity}

The remaining laws explain why the search stops at degree three and why a single monomial cannot hit the prime value $137$. This keeps the bound as a structural statement rather than a hidden search parameter.

The justification for the degree bound is structural, not merely numerical. One cannot simply say that every monomial of degree at least four already exceeds $137$: that statement is false in raw arithmetic, since expressions such as $\chi^4 = 16$, $\chi^4 d = 48$, $\chi^4 E = 96$, and $\chi^4 V = 64$ remain below $137$. The real point is different. The graph itself fixes the natural local complexity scale: $K_4$ has degree $d = 3$, and this same value appears as the simplex depth used in the coupling law. Monomials of degree at most three therefore exhaust the primitive local observables naturally generated by the graph's own interaction depth. Going beyond degree three does not reveal a new local layer of $K_4$; it merely iterates already available invariants into higher-order composites.

This matters for the epistemic status of the argument. At the standard set by the beginning of the book, elimination must always act on an explicitly delimited class of alternatives. That is exactly what happens here: the sector under classification is the class of primitive local observables on $K_4$, and the graph's own degree fixes that class at complexity $\le 3$. Within that class, the arithmetic classification becomes sharp. The only admissible two-term completion of the prime value $137$ is $128 + 9 = V^3\chi + d^2$. Higher-degree terms are therefore not being ruled out as meaningless formal expressions in general; they are being ruled out from this particular elimination problem because they lie outside the primitive observable sector being classified.

This cutoff cannot remain a verbal convention. If the primitive sector were left implicit, the bound $\deg \le d$ could be mistaken for a search parameter chosen after $137$ was already visible. The graph itself blocks that escape. The simplex depth is the local interaction depth of $K_4$, and \hyperref[law:k4:simplex-degree-match]{Law~16B.5} identifies that depth with the graph degree. Primitive observables therefore stop exactly where iteration begins.

\textbf{Constraint:} The admissible local sector must be delimited by graph structure alone. A bound introduced only to hit $137$ would destroy the elimination claim.

\textbf{Construction:} A primitive observable is represented by a monomial in the invariant basis $\{V,E,d,\chi,\kappa\}$ together with a proof that its total degree does not exceed $\texttt{simplex-degree}$. Since $\texttt{simplex-degree} \equiv d$, the cutoff is read directly from $K_4$.

\textbf{Consequence:} Degree-$4$ terms such as $\chi^4$ are not forbidden as expressions. They are classified as iterated composites outside the primitive sector. The $\alpha^{-1}$ decomposition survives this filter: $V^2\kappa$ has degree $3$, and $d^2$ has degree $2$.
...
\label{sec:discrete-continuum:epistemic-status}

The classification results above establish the following census:

\begin{enumerate}
\item \textbf{$\alpha^{-1} = 137$: forced by elimination.} Once the graph, invariant ledger, and primitive cutoff are fixed, the coupling sector contains exactly one admissible survivor. Among all two-term degree-$\le 3$ completions, only $V^d \chi + d^2 = 128 + 9$ reaches the prime value. No alternative passes the filter.

\item \textbf{Loop numerator $11$: forced by elimination.} The loop sector is finite and explicit. Among all $\{0,1\}$-linear combinations of $\{E, d, \chi\}$, exactly one satisfies irreducibility ($\gcd = 1$ with~$72$), completeness, and cross-scale consistency. The partial candidate $d + \chi = 5$ collapses on completeness. Only $E + d + \chi = 11$ survives.

\item \textbf{Denominators $72$, $576$, $274$: derived.} These do not require a separate survivor hunt. They are canonical products of quantities already forced upstream: $V \!\cdot\! E \!\cdot\! d$, $V \!\cdot\! E \!\cdot\! d \!\cdot\! \kappa$, $2\alpha^{-1}$. Once the factors survive, the products survive with them.

\item \textbf{Proton bare mass $1836$: forced by elimination after reduction.} The mass sector separates compactification content from local graph content. The compactified spinor survivor is already fixed at $F_2 = 17$, and the remaining cofactor reduces through $\chi d = E$ to the primitive edge channel. There, among all degree-$\le 3$ monomials in $\{E,d\}$, exactly one evaluates to $108$, namely $E^2 d$. The full product $F_2 \cdot E^2 d = 1836$ is therefore forced.

\item \textbf{Muon bare mass $207$: forced by elimination.} The muon sector survives only if its two channels remain distinct: geometric square and mixed compactification sum. Among the degree-only candidates $1,d,d^2,d^3$, exactly one evaluates to $9$. Among the additive candidates $0,E,F_2,E+F_2$, exactly one evaluates to $23$. Their product is therefore forced as $9 \cdot 23 = 207$.

\item \textbf{Tau/muon ratio $17$: forced by compactification elimination.} The terminal ratio sits in the compactification channel itself. That channel carries only two candidates: the raw spinor count $16$ and its one-point compactification $17$. Only one survives the terminal value filter. The survivor is $F_2 = 17$.
\end{enumerate}

The correction chain is now closed for $\alpha^{-1}$, for the loop numerator, for the denominators, and for the bare proton, muon, and tau sectors. What changes from one mass sector to the next is not the standard of proof, but the channel through which the surviving structure is allowed to speak. In the proton sector the graph first sheds its topological clothing and reveals an edge-channel core. In the muon sector the story splits into two visibly different motions: one geometric, one compactificatory, and the value survives only when those motions are kept separate until the end. In the tau sector even that split disappears: only the compactification residue is left on the table. None of these moves is an escape hatch. Each is forced by the way the graph freezes some degrees of freedom and leaves exactly one live channel behind.



\subsubsection*{Mass-sector admissibility sector}

The proton value $1836$ cannot be allowed to enter as a decorative product assembled after the fact. By the time the mass sector is reached, the same eliminative discipline that fixed $\alpha^{-1}$ is already in force here as well: new compactification content must be separated from local graph content, and whatever survives locally must survive inside an explicitly delimited admissibility sector. Physically, the reader should picture a two-stage filter. First, the genuinely new compactification content is isolated so that it cannot be smuggled back into local graph arithmetic. Then the local residue is forced to show its face inside the primitive channel already carved out by the graph. If that separation fails, the proton factorization collapses into bookkeeping.

What the graph leaves standing is a two-layer channel and nothing else. One layer contributes the genuinely new compactification survivor $F_2 = 17$. The other contributes a local cofactor that must already be present in the primitive observable sector once the topological route has been reduced through $\chi d = E$. In more concrete terms: one hand carries the compactification prime, the other hand carries the smallest local cofactor the graph cannot eliminate. The task is therefore not to decorate $1836$ with factors, but to expose the only factorization that survives this split.

\textbf{Constraint:} The mass observable must separate genuinely new compactification content from local graph content. If these layers are mixed, the factorization ceases to be structural.

\textbf{Construction:} The new factor is $F_2 = \mathrm{suc}(\mathrm{clifford\text{-}dimension})$. The local cofactor sector consists of primitive monomials whose reduced value is the edge-channel cofactor $E^2 d = 108$. The higher-degree presentation $\chi^2 d^3$ is retained only as a derivable route into that reduced cofactor.

\textbf{Consequence:} The proton formula is no longer a floating product. It lies in an explicit admissibility sector: compactification prime times reduced primitive cofactor. Any rival factorization would have to blur this separation, reintroduce non-primitive local structure, or abandon the compactification channel already fixed upstream. The reduced cofactor is then classified exhaustively below.
...
\subsubsection*{Reduced edge-channel elimination}

Reduction cannot stop at a slogan either. Once the cofactor has been pushed through $\chi d = E$, no hidden reservoir of alternatives remains. The surviving local channel is finite and exposed. There are only ten monomials in the edge channel $\{E,d\}$ of total degree at most three. If more than one of them hit $108$, the proton cofactor would remain ambiguous. If exactly one survives, the ambiguity is shattered and the edge channel closes.

\textbf{Constraint:} The reduced cofactor must live in the primitive edge channel and remain within the graph-forced cutoff $\deg \le d = 3$.

\textbf{Construction:} Enumerate all degree-$\le 3$ edge-channel monomials, evaluate each on $K_4$, and classify the unique survivor at value $108$.

\textbf{Consequence:} $E^2 d$ is not a convenient rewrite of the cofactor. It is the only reduced primitive edge-channel monomial that survives the value filter. Every weaker alternative is eliminated inside the finite case space itself. The proton mass is therefore forced as $F_2 \cdot E^2 d = 17 \cdot 108 = 1836$.
...
\subsubsection*{Muon mixed-channel elimination}

The muon sector cannot be left at the level of a suggestive product either. Once the proton has been forced, the next pressure point is obvious: is $207 = d^2(E+F_2)$ equally unavoidable, or is this the first place where convenience re-enters? The graph blocks that escape by forcing another finite sector. This time the structure does not collapse into a single multiplicative channel immediately. Instead it visibly splits into two pieces that do different jobs. The geometric contribution measures pure local repetition along the degree axis. The compactification contribution measures the extra residue that appears only when the edge channel and the compactified spinor channel are allowed to meet additively. Both spaces are small enough to classify exhaustively, so nothing can hide behind informal plausibility.

\textbf{Constraint:} The muon observable must keep the local geometric square distinct from the compactification correction. Repeating either channel would double-count structure and destroy the decomposition.

\textbf{Construction:} Enumerate the degree-only candidates $1,d,d^2,d^3$ and the mixed additive candidates $0,E,F_2,E+F_2$. Exactly one candidate in the first list evaluates to $9$, and exactly one candidate in the second evaluates to $23$.

\textbf{Consequence:} The product $d^2(E+F_2)$ is not a guess. It is the only product assembled from the unique geometric-square survivor and the unique mixed additive survivor. Any alternative muon factorization would have to alter one of those two finite survivor classes, and neither class has any room left. The muon bare mass is therefore forced as $207$.
...
\subsubsection*{Tau compactification elimination}

The tau sector is smaller, but it cannot be treated more loosely. Once the compactification channel has appeared in both proton and muon formulas, the terminal ratio $\tau/\mu = 17$ must survive by the same standard or the chain weakens at its end. Here the candidate space is minimal, and that is exactly why the section matters. All geometric clutter has already burned away. Nothing remains but the compactification channel itself: the raw spinor count and its one-point compactification. If both remained viable, the terminal ratio would still carry unresolved slack. If only one survives, the compactification channel closes without residue.

\textbf{Constraint:} The terminal lepton ratio must come entirely from the compactification channel, with no leftover geometric factor.

\textbf{Construction:} Enumerate the two compactification candidates: the raw spinor count $\mathrm{Cl}(\mathbb{R}^4)$ dimension $= 16$, and its one-point compactification $F_2 = 17$.

\textbf{Consequence:} The tau/muon ratio is not merely equal to $F_2$. It is the unique survivor of the compactification channel. The alternative value $16$ is not approximately wrong or philosophically weaker; it is eliminated inside the only candidate space available. The terminal ratio is therefore forced as $17$.
...
%───────────────────────────────────────────────────────────────────────────
\section{Physics Presupposes Distinction}
\label{sec:discrete-continuum:physics-presupposes-distinction}
%───────────────────────────────────────────────────────────────────────────

The final asymmetry can no longer remain implicit. Up to this point the book has forced the physical record from the invariant spine of $K_4$. A weaker reader could still try to reverse the dependence and say that physics is the deeper object, while $K_4$ is only one successful encoding of it. That escape collapses at the type level.

A physical record here is not a free-standing bag of measured numbers. It is threaded together by identity witnesses: the anchor tying dimension to $K_4$, the chain laws transporting that anchor through every other field, and the closure proofs showing that rival admissible records collapse to the same object. If identity is removed, these fields are not weakened. They become uninhabitable. Physics therefore does not generate distinction. It consumes the logical infrastructure distinction has already forced.

\textbf{Constraint:} An admissible physical record already contains equality data. The anchor and chain constraints of \texttt{K4PhysRep} presuppose the identity type, and the identity type presupposes the distinction-theoretic base established at the beginning of the book. If distinction were denied, the physics record would not become deeper; it would become uninhabitable.

\textbf{Construction:} Extract the record's own equality witness and use it to state the direction of dependence in two forms. First, every inhabitant of \texttt{K4PhysRep} yields a distinction witness. Second, assuming $\neg\,\mathit{Distinction}$ annihilates every purported physical record, so physics without distinction collapses to $\bot$.

\textbf{Consequence:} The dependency arrow cannot be reversed. Physics is an invariant of $K_4$, but $K_4$ is not reconstructed from a prior physics layer. The physical record already lives inside the logical world distinction opened. Physics as fundament is therefore eliminated by self-presupposition.

This does not close the book by withdrawing from physics into logic. It closes the directional ambiguity that would otherwise remain at the threshold. The route from distinction to $K_4$ to the physical record is now fixed in one direction only. What remains is not another internal theorem, but the outward question that this fixed direction leaves behind: what follows once physics has been shown to depend on distinction rather than ground it?

\phantomsection\label{law:discrete-continuum:physics-presupposes-distinction}
\phantomsection\label{law:discrete-continuum:physics-without-distinction-uninhabitable}

\begin{code}
-- § Any admissible K₄ physics record already consumes distinction.
physics-presupposes-distinction : (p : K4PhysRep) → Distinction
physics-presupposes-distinction p with K4PhysRep.anchor p
... | refl = Two-distinction

-- § Denying distinction makes the physics record uninhabitable.
physics-without-distinction-uninhabitable : ¬ Distinction → K4PhysRep → ⊥
physics-without-distinction-uninhabitable noDistinction p with K4PhysRep.anchor p
... | refl = noDistinction Two-distinction

-- § Every physical record therefore implies the irrefutability of distinction.
physics-presupposes-distinction-irrefutable : (p : K4PhysRep) → ¬¬ Distinction
physics-presupposes-distinction-irrefutable p noDistinction =
  physics-without-distinction-uninhabitable noDistinction p
\end{code}

\chapter{The Invitation}
\label{ch:invitation}

The chain has now been closed in the only direction that survives scrutiny. Distinction forces the two-class carrier; the carrier forces $K_4$; $K_4$ forces the invariant ledger; and the admissible physical record is shown, in the end, to presuppose the very distinction from which the chain began. What remains is therefore not a request for lenience. It is a ledger of the surviving debts. The proof no longer waits for completion, but it does leave obligations behind: comparison with rival physical explanations, sharpening of the mathematical form, and a philosophical account of what kind of necessity has been uncovered.

The invitation is the place where those debts are named without reopening the chain. It states, for each audience in turn, where resistance is still possible, where the real pressure now falls, and what kind of response would count as serious.

\section{To Physicists}
\label{sec:invitation:to-physicists}

This manuscript does not ask for deference to formalism, and it does
not claim a new empire of prediction. It makes a narrower and harsher
claim. Starting from distinction, and permitting only structures that
survive explicit elimination, one is driven to a graph, to an invariant
ledger, and then to a small cluster of computed quantities that can be
compared with physics. Those comparisons are now on the table in their
final form.

After the last asymmetry has been made explicit, the central physical
question is no longer whether physics secretly grounds the chain. That
path has been closed. The live question is whether the surviving match
to measured structure is accidental, derivative, or telling us something
real about how much of physics is already fixed before phenomenology
begins.

If the match is accidental, the standard of reply is clear. One must
produce another structure with comparable rigidity and no hidden freedom
that reaches the same constants with equal precision. If the match is
not accidental, the burden changes. One must explain why elimination on
such a small discrete spine reaches as far into the measured world as it
does.

The physical debt is therefore comparative rather than decorative. The
chain is not asking for applause. It is asking whether any equally rigid
route reaches the same measured structure, or whether this one is
already telling us that more of physics is fixed before model-building
begins than the usual story admits.

\section{To Mathematicians}
\label{sec:invitation:to-mathematicians}

The formal chain is closed, but closure is not the same as exhaustion.
What survives here is strong enough to be interrogated from several
directions at once: proof-theoretic, categorical, spectral, and
geometric. The invitation is therefore not to repair a missing proof,
but to decide whether the surviving structure admits a sharper and more
illuminating formulation.

Several pressure points are immediate:
...
The mathematics is self-contained. The open work is not to rescue the
argument from incompleteness, but to determine whether the same survivor
can be characterized in a form that is even more rigid, more general, or
more transparent.

This is the largest technical debt left by the manuscript. The theorem
chain stands, but its strongest surviving form may still be ahead of us:
a presentation in which classification becomes inevitability in an even
sharper sense than exhaustive proof can display.

\subsection{A Concrete Path: The Pure Derivation as a Short Paper}
\label{sec:invitation:pure-derivation-paper}

The physics claim cannot be evaluated by a community that has not first
accepted the formal derivation. These are separate questions, and they
require separate acts of verification.

A short companion paper---four to eight pages---addressed to the type
theory community and submitted to a venue such as TYPES, LICS, or CSL,
would isolate exactly the question that must be settled first:
\emph{Is the derivation of $K_4$ from the distinction formally
compulsory?} No physics. No $\alpha$. No spectral interpretation.
Only the bare chain: distinction, coverage, four-case classification,
drift acyclicity, presentation fixpoint, $K_4$ uniqueness.

Such a paper would serve three purposes:

\begin{enumerate}
\item \textbf{External validation.} Acceptance at a refereed venue
  establishes that the formal argument is not circular, not trivially
  blocked, and not dependent on hidden postulates. The Agda source is
  the proof artifact; the paper is the narrative wrapper that makes it
  accessible.

\item \textbf{Separation of concerns.} The physics community is right
  to demand formal credibility before evaluating the numerical claims.
  A published formal result provides exactly the external reference
  that the physics discussion needs.

\item \textbf{Pressure test.} If the derivation contains a gap---a step
  where an alternative survives that the book overlooks---a type-theory
  referee is best positioned to find it. Submitting the core argument to
  that scrutiny is not a formality. It is the most informative experiment
  available.
\end{enumerate}

The companion paper does not replace the present book. It extracts the
load-bearing chain, strips away the arithmetic and physics layers, and
presents the bare derivation in the language and conventions of its
natural audience. If it survives review, the book gains an external
anchor. If it does not, the failure will be informative in exactly the
way that matters.

\section{To Philosophers}
\label{sec:invitation:to-philosophers}

Every epoch produces a new observer and a new key to intelligibility.
One age trusts geometry. Another trusts experiment. Another trusts
symmetry, or information, or computation. This work belongs to a later
observer: one who asks what remains when every explanatory surplus is
forced to justify itself and only formal survival is allowed to stay.

That shift matters. The manuscript does not begin from world, mind, or
measurement and then ascend toward abstraction. It begins from the bare
fact of distinction and asks what can survive collapse. If physical
structure reappears at the end of that descent, then the philosophical
problem is no longer how mathematics is applied from outside. It is how
far the order of the world is already exposed by the conditions of
stable form.
...
\item What becomes of scientific realism if successive traditions are
  not competitors but partial witnesses of the same invariant order?
\end{itemize}

The philosophical debt is therefore neither ornament nor rescue. It is
to say what has happened when a structure reached by elimination behaves
less like a formal toy and more like an exposed layer of the world. If
that question is avoided, the proof remains intact, but its meaning is
left underdescribed.

Taken together, these are the debts that survive the end of the proof:
empirical comparison, mathematical sharpening, and philosophical
interpretation. None of them reopens the chain from distinction to
$K_4$ to the physical record. They begin only after that chain has been
forced. The conclusion therefore does not add one more theorem. It names
what kind of survivor has been left in our hands.

\chapter{Conclusion}
\label{ch:conclusion}

We began with a mark on a blank page---the simplest possible act:
separating something from nothing. Nothing in the chain that followed
was allowed to stand by taste, elegance, or convenience. The question
was harsher from the beginning: what remains if every unstable
alternative is forced to fall away?

By the end, even the final temptation to reverse the chain had been
eliminated. The physical record did not stand behind distinction as a
deeper source from which $K_4$ could be rebuilt. It stood downstream,
already inhabiting the equality and distinction structure it might have
claimed to explain. The dependence ran one way only.

Through the entire chain, the method stayed the same. Not invention,
but elimination. Not ornament, but survival. Equality had to survive.
Negation had to survive. Classification had to survive. Reachability,
presentation, fixpoint, graph, arithmetic, spectrum, observable: each
was admitted only where contradiction failed to destroy it.

What survived was small. A distinction. Two sides. Four cases. One
graph. A narrow arithmetic. A rigid spectrum. A short list of
invariants. That is why the result is unsettling. It does not arrive
as a grand architecture chosen for elegance. It arrives as residue.

No triumph should be claimed here. This work does not say that the
physicists were wrong and the logicians were right, or that the
mathematicians were early and the philosophers confused. It says
something more difficult: that they may all have been watching the same
constraint from different entrances.

Each epoch brings forth a new observer and a new key to knowledge. One
measures. One classifies. One formalizes. One asks what symmetry will
allow. One asks what computation will certify. This book belongs to
that last lineage. It asks a single question and refuses to relax it:
what if nothing remains except distinction, consistency, invariance,
and the structures they force?

Then almost everything disappears.

No new empire of prediction is announced here. No license is claimed
to outrank experiment. The modest claim is the stronger one: we do not
need to show more than that the great observations and achievements of
their respective traditions may be locally right together. Brouwer's
witness, Martin-Löf's discipline, Euler's count, Cayley's symmetry,
Noether's invariance, Curie's asymmetry, Dirac's severity, Feynman's
consistency of paths---none is overturned here. They survive.

What now survives with them is a threefold obligation. The empirical
question is comparative: whether any equally rigid route reaches the
same measured structure without concealed freedom. The mathematical
question is sharpening: whether the survivor can be recast in a form
still more inevitable than exhaustive classification. The philosophical
question is interpretive: what kind of necessity has been exposed if a
structure forced by elimination reaches so far into the world we
measure.

If this manuscript has earned any right to continue beyond its last
code line, it is because it leaves us with a cleaner question than the
one with which it began. Why should the survivor of formal elimination
resemble the measured world at all? Why should a structure forced by
logic meet the constants of physics where they are sharpest?

Those are not repairs to a broken argument. They are the proper debts of
one that has reached its limit without collapsing. The proof ends where
comparison, sharpening, and interpretation begin.

That is not a failure of the book. It is the honest end of the journey.
