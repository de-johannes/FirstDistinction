...
\title{
  ...
  \Large Graph Theory from a Primitive Principle in Agda\\[1em]
  \large Machine-Verified with \texttt{--safe --without-K}
}
...
\begin{abstract}
\noindent
This document presents, in Agda \texttt{--safe --without-K}:

\textbf{The claim:} From a single premise---``something can be distinguished from something''---the complete graph $K_4$ (tetrahedron) emerges as the unique stable structure. This is graph theory: 4 vertices, 6 edges, Euler characteristic $\chi = 2$.

\textbf{The construction:} One distinction ($D_0$) forces a second ($D_1$). Two force a third ($D_2$). Three force a fourth ($D_3$). At four, closure: every pair has a witness. $K_4$ is the minimal graph where this stability holds.

\textbf{The observation:} $K_4$ invariants match physics:
\begin{itemize}[nosep]
  \item Laplacian eigenspace dimension $\to d = 3$ (spatial dimensions)
  \item Euler $\times$ degree $\times$ vertices $\to \kappa = 8$ (Einstein coupling)
  \item Spectral formula $\to \alpha^{-1} = 137.036$ (fine structure, 0.00003\% error)
  \item Combinatorial formulas $\to$ particle mass ratios (0.008--1\% error)
\end{itemize}

\textbf{Status:} The mathematics is machine-verified. The physics correspondence is hypothesis---but one supported by remarkable numerical agreement across multiple independent quantities.
\end{abstract}
...
\subsection{The Unavoidable First Distinction \texorpdfstring{($D_0$)}{(D0)}}
...
\subsection{Memory Saturation and \texorpdfstring{$K_4$}{K4} Emergence}
...
\subsection{\texorpdfstring{$K_4$}{K4} Uniqueness}
...
\subsection{The \texorpdfstring{$K_4$}{K4} Laplacian and Eigenvalues}
...
\subsection{The Coupling Constant \texorpdfstring{$\kappa = 8$}{κ = 8}}
...
\subsection{The \texorpdfstring{K$_4$}{K4} Entanglement Identity}
...
\subsection{Robustness: Why Not \texorpdfstring{K$_3$}{K3} or \texorpdfstring{K$_5$}{K5}?}
