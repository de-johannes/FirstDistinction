...
\title{
  ...
  \large\normalfont Machine-Verified in Agda with \texttt{--safe --without-K}\\[0.5em]
  \normalsize Master Overview --- See FD-01 through FD-05 for Detailed Papers
}
...
\begin{abstract}
\noindent
\textbf{What this document proves} (Agda \texttt{--safe --without-K}, 7,938 lines):
...
\medskip
\textbf{Paper Series:} This overview is supplemented by six focused papers (61 pages total):
\begin{itemize}[nosep]
  \item FD-01: Genesis \& $K_4$ Uniqueness (8 pages)
  \item FD-02: Fine Structure Constant (7 pages)
  \item FD-03: Dimension 3+1 from Eigenvalues (9 pages)
  \item FD-04: Particle Mass Ratios (10 pages)
  \item FD-05: Time from Asymmetry (12 pages)
  \item FD-06: The Four-Part Proof Pattern (15 pages)
\end{itemize}
\end{abstract}
...
This is a \textbf{master overview} providing a broad introduction. For detailed proofs and analysis, see the individual papers:
...
\subsection{The Paper Series}

\begin{itemize}
  \item \textbf{FD-01: Genesis \& $K_4$ Uniqueness} --- Proves why exactly 4 distinctions are forced, why $K_3$ fails, why $K_5$ is unnecessary
  \item \textbf{FD-02: The Fine Structure Constant} --- Derives $\alpha^{-1} = 137.036\overline{036}$ from $K_4$ spectral structure
  \item \textbf{FD-03: Dimension 3+1} --- Shows eigenvalue structure $(0,4,4,4)$ corresponds to spacetime signature
  \item \textbf{FD-04: Particle Mass Ratios} --- Derives proton, muon, tau masses from $K_4$ combinatorics
  \item \textbf{FD-05: Time from Asymmetry} --- Proves Minkowski signature $(-,+,+,+)$ from genesis asymmetry
\end{itemize}
...
\textit{For complete genesis proof with captures relation and memory saturation, see FD-01.}
...
\subsection{Bridge to Differential Geometry}

\begin{keyinsight}
The graph Laplacian $L_{K_4}$ is the \textbf{discrete analogue} of the Laplace-Beltrami operator on a Riemannian manifold. This provides a rigorous bridge from $K_4$ to continuum geometry:

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Discrete ($K_4$)} & \textbf{Continuum (Riemannian)} \\
\midrule
Eigenvalues $\{0, 4, 4, 4\}$ & Spectrum of $\Delta$ \\
Multiplicity 3 & Dimension $d = 3$ \\
$\mathrm{Tr}(L) = 12$ & Scalar curvature $R = 12$ \\
6 edges & 6 independent components of $g_{\mu\nu}$ \\
Eigenvector to $\lambda=0$: $(1,1,1,1)$ & ``Time'' direction (drift) \\
\bottomrule
\end{tabular}
\end{center}

The 6 edges of $K_4$ correspond to the 6 independent components of a symmetric $4 \times 4$ metric tensor $g_{\mu\nu}$. The Einstein equations emerge via:
\[
G_{\mu\nu} + \Lambda g_{\mu\nu} = \kappa T_{\mu\nu} \quad \text{with } \Lambda = 3, \; \kappa = 8
\]
\end{keyinsight}
...
\begin{itemize}
  \item \texttt{FirstDistinction.agda} (7,938 lines)
  \item Compiles with \texttt{agda --safe --without-K}
  \item Zero axioms, zero postulates
  \item Available at: \url{https://github.com/de-johannes/FirstDistinction}
\end{itemize}

\textbf{Paper PDFs:} All five papers (FD-01 through FD-05) are available in the \texttt{pdf/} directory of the repository.
...
\textit{For complete eigenvalue analysis and uniqueness proof, see FD-03.}
...
\textit{For complete derivation with four-part validation, see FD-02.}
...
Predicted: $137 + \frac{4}{111} = 137.036\overline{036}$

Agreement: $\approx 0.000027\%$ (the denominator 111 = deg $\times$ (E²+1) is derived)

\medskip
\textbf{Note on the $+1$ in $E^2+1$:} This is \textit{not} an arbitrary fitting parameter. It follows the same \textbf{one-point compactification} pattern as:
\begin{itemize}[nosep]
  \item $V+1=5$ (vertices + centroid)
  \item $2^V+1=17$ (spinors + vacuum)
  \item $E^2+1=37$ (edge-pair couplings + asymptotic/free state)
\end{itemize}
The $+1$ represents the topological closure needed for the discrete-to-continuous transition. See \texttt{src/agda/Compactification.agda} for the formal derivation.

\medskip
\textbf{On the emergence of $\pi$:} The discrete $K_4$ contains no $\pi$---all its invariants ($V=4$, $E=6$, $\lambda=4$, $\chi=2$) are rational. $\pi$ \textit{emerges} through the discrete-to-continuous transition:
\begin{itemize}[nosep]
  \item Embedding $K_4$ into $\mathbb{R}^3$ introduces spherical geometry ($4\pi r^2$)
  \item The $S_4$ symmetry (24 elements) approximates $SO(3)$, which contains $\pi$
  \item $\pi$ is the ``price of continuity''---it appears whenever we pass from $\mathbb{Q}$ to $\mathbb{R}$
\end{itemize}

\medskip
\textit{While the $+1$ is mathematically derived, no causal physical connection is proven.}
\end{observation}
...
\textit{For complete derivation with exclusivity proofs, see FD-04.}
...
\textit{For complete derivation of Minkowski signature, see FD-05.}
...
$\alpha^{-1}$ & $137+\frac{4}{111}$ & 137.036 & 0.000027\% \\
...
\begin{itemize}
  ...
  \item The matches are precise (0.000027\% to 1.2\%)
...
The following figures illustrate the key mathematical results. All are computed directly from $K_4$ structure.

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{../docs/assets/images/fig2_alpha_uniqueness.png}
\caption{\textbf{Why $K_4$ is unique.} The spectral formula $\lambda^d \chi + \text{deg}^2$ for complete graphs $K_n$. Only $K_4$ produces a value near 137. This is not fine-tuning.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{../docs/assets/images/fig4_genesis.png}
\caption{\textbf{The forced emergence of $K_4$.} Each step is necessary: $D_1$ distinguishes $D_0$ from its absence, $D_2$ witnesses the $(D_0, D_1)$ pair, $D_3$ closes the structure.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{../docs/assets/images/fig5_compactification.png}
\caption{\textbf{One-point compactification.} The +1 pattern: $V+1=5$, $2^V+1=17$, $E^2+1=37$. All compactified values are prime. This is the mathematical price for transitioning from discrete $K_4$ to continuous physics.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{../docs/assets/images/fig3_precision.png}
\caption{\textbf{Precision.} Our formula gives $\alpha^{-1} = 137 + 4/111 = 137.036036...$, matching CODATA 2022 ($137.035999$) to 0.000027\%.}
\end{figure}
...
\bibitem{fd01}
J. Wielsch, \textit{FD-01: Genesis \& $K_4$ Uniqueness}, 
FirstDistinction Paper Series (2025).

\bibitem{fd02}
J. Wielsch, \textit{FD-02: Fine Structure Constant from $K_4$}, 
FirstDistinction Paper Series (2025).

\bibitem{fd03}
J. Wielsch, \textit{FD-03: Dimension 3+1 from Eigenvalues}, 
FirstDistinction Paper Series (2025).

\bibitem{fd04}
J. Wielsch, \textit{FD-04: Particle Mass Ratios from $K_4$ Combinatorics}, 
FirstDistinction Paper Series (2025).

\bibitem{fd05}
J. Wielsch, \textit{FD-05: Time from Asymmetry}, 
FirstDistinction Paper Series (2025).

\bibitem{fd06}
J. Wielsch, \textit{FD-06: The Four-Part Proof Pattern}, 
FirstDistinction Paper Series (2025).
