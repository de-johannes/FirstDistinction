...
\subsection{Brücke zur Differentialgeometrie}

\begin{einsicht}
Die Graph-Laplace-Matrix $L_{K_4}$ ist das \textbf{diskrete Analogon} des Laplace-Beltrami-Operators auf einer Riemannschen Mannigfaltigkeit. Dies liefert eine rigorose Brücke von $K_4$ zur Kontinuumsgeometrie:

\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Diskret ($K_4$)} & \textbf{Kontinuum (Riemannsch)} \\
\midrule
Eigenwerte $\{0, 4, 4, 4\}$ & Spektrum von $\Delta$ \\
Multiplizität 3 & Dimension $d = 3$ \\
$\mathrm{Tr}(L) = 12$ & Skalarkrümmung $R = 12$ \\
6 Kanten & 6 unabh. Komponenten von $g_{\mu\nu}$ \\
Eigenvektor zu $\lambda=0$: $(1,1,1,1)$ & "`Zeit"'-Richtung (Drift) \\
\bottomrule
\end{tabular}
\end{center}

Die 6 Kanten von $K_4$ entsprechen den 6 unabhängigen Komponenten eines symmetrischen $4 \times 4$ metrischen Tensors $g_{\mu\nu}$. Die Einstein-Gleichungen emergieren via:
\[
G_{\mu\nu} + \Lambda g_{\mu\nu} = \kappa T_{\mu\nu} \quad \text{mit } \Lambda = 3, \; \kappa = 8
\]
\end{einsicht}
...
Vorhergesagt: $137 + \frac{4}{111} = 137{,}036\overline{036}$

Übereinstimmung: $\approx 0{,}000027\%$ (der Nenner 111 = deg $\times$ (E²+1) ist abgeleitet)
...
\medskip
\textbf{Zur Emergenz von $\pi$:} Das diskrete $K_4$ enthält kein $\pi$---alle seine Invarianten ($V=4$, $E=6$, $\lambda=4$, $\chi=2$) sind rational. $\pi$ \textit{emergiert} durch den Übergang diskret$\to$kontinuierlich:
\begin{itemize}[nosep]
  \item Einbettung von $K_4$ in $\mathbb{R}^3$ führt sphärische Geometrie ein ($4\pi r^2$)
  \item Die $S_4$-Symmetrie (24 Elemente) approximiert $SO(3)$, die $\pi$ enthält
  \item $\pi$ ist der "`Preis der Kontinuität"'---es erscheint bei jedem Übergang von $\mathbb{Q}$ zu $\mathbb{R}$
\end{itemize}
...
\begin{itemize}
  ...
  \item Die Übereinstimmungen sind präzise (0,000027\% bis 1,2\%)
...
\newpage
\section*{Anhang: Visualisierungen}

Die folgenden Abbildungen illustrieren die zentralen mathematischen Resultate. Alle werden direkt aus der $K_4$-Struktur berechnet.

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{../figures/fig2_alpha_uniqueness.pdf}
\caption{\textbf{Warum $K_4$ einzigartig ist.} Die Spektralformel $\lambda^d \chi + \text{deg}^2$ für vollständige Graphen $K_n$. Nur $K_4$ liefert einen Wert nahe 137. Das ist kein Feintuning.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{../figures/fig4_genesis.pdf}
\caption{\textbf{Die erzwungene Entstehung von $K_4$.} Jeder Schritt ist notwendig: $D_1$ unterscheidet $D_0$ von seiner Abwesenheit, $D_2$ bezeugt das $(D_0, D_1)$-Paar, $D_3$ schließt die Struktur.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{../figures/fig5_compactification.pdf}
\caption{\textbf{Einpunkt-Kompaktifizierung.} Das +1-Muster: $V+1=5$, $2^V+1=17$, $E^2+1=37$. Alle kompaktifizierten Werte sind prim. Das ist der mathematische Preis für den Übergang vom diskreten $K_4$ zur kontinuierlichen Physik.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{../figures/fig3_precision.pdf}
\caption{\textbf{Präzision.} Unsere Formel gibt $\alpha^{-1} = 137 + 4/111 = 137{,}036036...$, übereinstimmend mit CODATA 2022 ($137{,}035999$) auf 0{,}000027\%.}
\end{figure}
