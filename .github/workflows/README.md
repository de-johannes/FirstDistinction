...
**Jobs:**
1. **agda-verification:** Compile `FirstDistinction.lagda.tex` with `--safe --without-K --no-libraries`
...
**Steps:**
1. Compile `FirstDistinction.lagda.tex` with `--safe --without-K --no-libraries`
2. Check for holes `{!!}`
...
```
FirstDistinction.lagda.tex  # Complete proof: D₀ → G_μν = 8T_μν
